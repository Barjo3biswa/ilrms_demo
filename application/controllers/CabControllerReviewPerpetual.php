<?php
class CabControllerReviewPerpetual extends MY_Controller
{
  public function __construct()
  {
    ...
    $this->load->model('CabReviewPerpetualModel');
  ...
  public function createCabId() {
    ...
    if ($this->designation == DPT_JS && $this->input->post('option') == null ){
      $data['user_assigned_dist'] = $this->CabReviewPerpetualModel->getDeptUserDistList()->result();
      $data['_view'] = 'cab/createCabIdReviewPerpetual';
      $this->load->view('layouts/main', $data);
    }
    else if ($this->designation == DPT_JS && $this->input->post('option') == 'edit') {      
      ...
      $getSelectedDist = $this->db->query("SELECT dist_code FROM cab_id_list WHERE cab_id=?",
                            array($cab_id))->result();
      $user_assigned_districts = $this->CabReviewPerpetualModel->getDeptUserDistList()->result();
  ...
  public function generateCabId() {
    ...
    $this->form_validation->set_rules('cab_type', 'Cabinet Type', 'trim|required');
  ...
  public function toBeFinalizeCabId() {
    if ($this->session->userdata('designation') == DPT_JS) {     
      // $data['user_assigned_dist'] = $this->CabReviewPerpetualModel->getDeptUserDistList()->result(); 
      $data['_view'] = 'cab/toBeFinalizeCabReviewPerpetual';
      $this->load->view('layouts/main', $data);
    } else {
  ...
  public function getCasesByCabId() 
  {
    ...
    $result = $this->CabReviewPerpetualModel->getCasesByCabId($cab_id, $status);
  ...
  public function finalApproveCabId() {
    if ($this->session->userdata('designation') == DPT_JS) {     
      // $data['user_assigned_dist'] = $this->CabReviewPerpetualModel->getDeptUserDistList()->result(); 
      $data['_view'] = 'cab/finalApprovalCabIdReviewPerpetual';
      $this->load->view('layouts/main', $data);
    } else {
      echo "User Not Authorized to View this Page";
    }
  ...
  public function getCabIdByUserDistrict() {
    ...
    $memo_list = $this->CabReviewPerpetualModel->getCabIdListFromMaster($start, $length, $order,$status);
    //var_dump($memo_list);exit;
    ...
    if(!empty($memo_list)) {
      ...
      if($memo_list['total_records'] > 0){
        ...
        foreach($data_rows as $row) {
          ...
          $cab_type = $this->db->query("SELECT cab_type  FROM cab_id_list WHERE cab_id=?", array($row->cab_id))->row()->cab_type;

          $created_at = date('d-m-Y',strtotime($row->created_at));

          $link = base_url() . "index.php/CabControllerReviewPerpetual/getListOfCasesByCabId?cab_id=".$row->cab_id;
  ...
  public function getListOfCasesByCabId()
  {
    ...
    $memo_name = $this->basundharaMb2Reviewmodel->getMemoNameByCabId($this->db,$cab_id);
    ...
    // $data['cases'] = $query;
    $data['_view'] = 'cab/viewCaseDetailsByCabIdReviewPerpetual';
    $this->load->view('layouts/main', $data);
  }
    ...
    if(!empty($memo_list)) {
      ...
      if($memo_list['total_records'] >  0){
        ...
        foreach($data_rows as $row) {
          ...
          $link = base_url() . "index.php/CabControllerReviewPerpetual/viewGeneratedMemo?cab_id=".$row->cab_id;
          $view_memo = "<a href=".$link." class='btn btn-sm btn-warning' target='_blank'><i class='fa fa-eye'></i> &nbsp;View Memo</a>";

         $linkDoc = base_url() . "index.php/CabControllerReviewPerpetual/viewGeneratedMemoDoc?cab_id=".$row->cab_id;
          ...
          $link1 = base_url() . "index.php/BasundharaMb2Review/casesListForFinalApprovalByDept?cab_id=".$row->cab_id;
          ...
          $link4 = base_url() . "index.php/CabControllerReviewPerpetual/downloadGeneratedNotification?cab_id=".$row->cab_id;
          ...
          $json[] = array(
            ...
            '<strong class="text-success"> '. $cab_type .'</strong>',
  ...
  public function GenerateCabMemo()
  {
    ...
    $cab_type = $this->db->query("select cab_type from cab_id_list where cab_id=?",array($cab_id_memo))->row()->cab_type;
    ...
    $data['total_prop'] = 0;
    $data['reverted_cases_count'] = 0;
    $data['recomended_cases_count'] = 0;

    $data['cab_memo_list'] = $this->db->query("select * from cab_memo_list where cab_id=?",array($cab_id_memo))->result();
    ...
    if (!empty($res) && $res != null && $res != "") {
      ...
      $data['reverted_cases_count_text'] = $this->utilclass->numberToWords($data['reverted_cases_count']);
      $data['recomended_cases_count_text'] = $this->utilclass->numberToWords($data['recomended_cases_count']);
    }
    ...
    foreach($distMeeting as $dist)
    {
      ...
      {
          if($cab_type =='REVIEW'){
            $this->load->view('cabinet_Review', $data);  
          }elseif($cab_type =='PERPETUAL'){
            $this->load->view('cabinet_perpetual', $data);  
          }else{
            echo "#Err21022025 001 Cab type Not Found";
            exit;
          }
  ...
  public function getAllCasesUnderCabId() {
    ...
    $cases_list = $this->basundharaMb2Reviewmodel->getAllCasesbyCabId($this->db,$start, $length, $order,$cab_id,$user_code,$meeting_no,$dist_code,$service_code);
                  ...
                  foreach ($allSelectedList as $row) {
                      ...
                      $parts = explode("@", $row);
                        ...
                        list($case_no, $dist_code, $meeting_id) = $parts;
                          ...
                          $tstatus3 = $this->db->insert('deleted_cab_memo_list', $deleted_cab_memo_details); 
                          if ($tstatus3 <= 0)
                          {
                              $this->db->trans_rollback();
                              log_message("error", "#EKHIMBDO0031, Error in insert on deleted_cab_memo_list table with query- ". json_encode($this->db->last_query()));
                              echo json_encode(array(
                                'responseType' => 1,
                                'message' => '#EKHIMBDO0031 Failed to Remove Cases from Cabinet Memo',

                            ));
                            return false;
                          }
                            
                          $deleteCases = $this->basundharaMb2Reviewmodel->deleteCasesFromCabMemo($cabMemoData);
                          if ($deleteCases <= 0) {
                              $this->db->trans_rollback();
  ...
  {    
    ...
    // $list_of_generated_cab_memo = $this->CabReviewPerpetualModel->getCabIdListFromMaster($status);
    // log_message('error', '#950: '.json_encode($list_of_generated_cab_memo));
    // $total_records = $list_of_generated_cab_memo->num_rows();

    $list_of_generated_cab_memo = $this->CabReviewPerpetualModel->getCabIdListFromMaster($start, $length, $order,$status);
    ...
    if(!empty($memo_list)) {
      ...
      if($memo_list['total_records'] >  0){
        ...
        foreach($data_rows as $row) {
          ...
          $link3 = base_url() . "index.php/CabControllerReviewPerpetual/getListOfFinalApprovedCasesByCabId?cab_id=".$row->cab_id;
          ...
          $link4 = base_url() . "index.php/CabControllerReviewPerpetual/viewGeneratedNotification?cab_id=".$row->cab_id;
...
/////////VGR/ PGR///////////
  ...
  public function createVGRCabId() {
    ...
    if ($this->designation == DPT_JS && $this->input->post('option') == null ){
      // $data['user_assigned_dist'] = $this->CabReviewPerpetualModel->getDeptUserDistListVGR()->result();
      ...
      $getSelectedDist = $this->db->query("SELECT dist_code FROM cab_id_list WHERE cab_id=?",
                            array($cab_id))->result();
      // $user_assigned_districts = $this->CabReviewPerpetualModel->getDeptUserDistListVGR()->result();
      echo json_encode(array(
        'responseType'  => 2,
        'cab_id'        => $sql->cab_id,
        'memo_name'     => $sql->cab_memo_name,
  ...
  {    
    ...
    $list_of_generated_cab_memo = $this->CabReviewPerpetualModel->getCabIdListFromMasterVGR($start, $length, $order,$status);
  ...
  public function toBeFinalizeVGRCabId() {
    ...
    $memo_list = $this->CabReviewPerpetualModel->getCabIdListFromMasterVGR($start, $length, $order,$status);
    ...
    if(!empty($memo_list)) {
      ...
      if($memo_list['total_records'] >  0){
        ...
        foreach($data_rows as $row) {
          ...
          $link3 = base_url() . "index.php/BasundharaReviewPerpetual/casesListForFinalApprovalByDeptVGR?cab_id=".$row->cab_id;
          ...
          $link4 = base_url() . "index.php/CabControllerReviewPerpetualReviewPerpetual/downloadGeneratedNotification?cab_id=".$row->cab_id;
          $download_notification = "<a href=".$link4." class='btn btn-sm btn-success' ><i class='fa fa-download'></i> &nbsp;Download Notification</a>";
  ...
  public function GenerateNotificationForSign()
  {
    ...
    $data['cab_id_memo'] = $cab_id_memo = $this->input->post('cab_id');
      ...
      if($cab_type == 'REVIEW'){
          $this->load->view('notification_for_sign_review', $data); 
      }elseif($cab_type == 'PERPETUAL'){
          $this->load->view('notification_for_sign_perpetual', $data); 
      }else{
        echo "#Err21012025 : Cab Type Not Found";
        exit;
      }
  ...
  public function SavePDFNotificationCopyVgr()
  {
    ...
    {   
        ...
        $this->basundharaMb2Reviewmodel->downloadExcelReport($file_name, $final);
    }
