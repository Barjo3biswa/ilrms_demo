<?php
class OfflineSettlement extends MY_Controller
{
    ...
    public function getAllCasesListByDept()
    {
        ...
        if ($this->session->userdata('designation') == DPT_JS || $this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $data['user_dist']      = $this->OfflineSettlementModel->getDeptUserDistListWithCaseCount();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['_view'] = 'offlineSettlement/dept/AllPendingCaseListUnderDpt';
                $this->load->view('layouts/main', $data);
            } else {

                $this->db2 =  $this->dbswitch2($dist_code);
                $meeting = $this->OfflineSettlementModel->getMeetingListByDist($this->db2,$dist_code);
                ...
                $cabIdList = $this->OfflineSettlementModel->getCabinetIdList($dist_code,$user_code)->result();
    ...
    function request($service)
    {
        ...
        $settlement['pending'] = $this->OfflineSettlementModel->getDepartmentRequest($service, $dist_list);
    ...
    {
        //var_dump($_SESSION);
        $this->load->model('basundhara/OfflineSettlementModel');
        $settlement['pending'] = $this->OfflineSettlementModel->alldepartmentApprovedCases($service);
    ...
    function settlementBasu()
    {
        ...
        $settlement['dist_code'] = $dist_code;
        $this->db2 = $this->dbswitch2($dist_code);
        ...
        $settlement['settlement_basic'] = $this->OfflineSettlementModel->getSettlementBasic($case_no);
        $settlement['settlement_applicant']  = $this->OfflineSettlementModel->getSettlementApplicant($case_no);
        $settlement['caseDetails']  = $this->OfflineSettlementModel->getOfflineSdlacDetailsByCaseNo($case_no);

        $settlement['settlement_dag_details'] = $this->OfflineSettlementModel->getSettlementDagDetails($case_no);
        $settlement['settlement_dag_area'] = $this->OfflineSettlementModel->getSettlementDagArea($case_no);
        $settlement['settlement_proceeding'] = $this->OfflineSettlementModel->getSettlementProceeding($case_no);
        $settlement['settlement_ap_lmnote'] = $this->OfflineSettlementModel->getSettlementLmNote($case_no);
        $settlement['supportive_document'] = $this->OfflineSettlementModel->getSupportiveDocuments($case_no);
        $settlement['proceedings']   = $this->OfflineSettlementModel->getSettlementProceeding($case_no);
        // Newly added on 08/09/2022
        $settlement['applicants_buyers']   = $this->OfflineSettlementModel->getAllApplicantBuyers($case_no);
        $settlement['applicants_owners']   = $this->OfflineSettlementModel->getAllApplicantOwners($case_no);
        $settlement['applicants_encroacher']   = $this->OfflineSettlementModel->getAllApplicantEncroacher($case_no);
        $settlement['applicants_riotee_nok']   = $this->OfflineSettlementModel->getAllApplicantRioteeNok($case_no);

        // New Added Settlemet Reservation Details
        $settlement['roadside_reservation']   = $this->OfflineSettlementModel->getSettlementRoadsideReservation($case_no);
        $settlement['family_reservation']   = $this->OfflineSettlementModel->getSettlementFamilyReservation($case_no);

        

        $settlement['premium_data']  = $this->OfflineSettlementModel->getSettlementPremium($case_no);
        $settlement['landmark_data'] = $this->OfflineSettlementModel->getSettlementDagArea($case_no);
        $settlement['possession_data']   = $this->OfflineSettlementModel->getAllPossessionDetails($case_no);


        //*******getting the deleted settlement_dag_details data from settlement_deleted_data table */
        $deletedEnc=$this->OfflineSettlementModel->getDeletedEncroacher($case_no);
        ...
        $deletedDags=$this->OfflineSettlementModel->getDeletedDags($case_no);
        $deletedData = array();
        ...
        $getJsonBackup = $this->OfflineSettlementModel->getJsonDataFromBackup($case_no);
        ...
        if (isset($case_no)) {
            $settlement['service_name'] = 'OFFLINE SETTLEMENT KHAS LAND';
            $settlement['_view'] = 'offlineSettlement/Dept/SettlementKhasLandView';
        } else {
            $settlement['_view'] = 'offlineSettlement/Dept/CaseNotFound';
        }
        $this->load->view('layouts/main', $settlement);
    }
    //////////////////////////
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($dist_code == '' and $caseNo == ''  and $applicationNo == '' and $serviceType == '' and $appStatus == '' and $pendingOffice == '' and $fromDate == '' and $toDate == '' and $cir_code == '') {
                ...
                if ($caseNo != '') {
                    ...
                    $cases = $this->OfflineSettlementModel->getCasesByCaseNo($caseNo);
                    ...
                    $cases = $this->OfflineSettlementModel->getCasesByApplicationNo($applicationNo);
                    $data['cases']      = $cases->result();
                    $data['casesCount'] = $cases->num_rows();
                    ...
                    if ($fromDate != '' and $toDate != '') {
                        ...
                        $cases = $this->OfflineSettlementModel->getCasesByRespectedDataWithDateRage($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
                        ...
                        $cases = $this->OfflineSettlementModel->getCasesByRespectedData($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
    ...
    public function searchProposalWIthData()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->OfflineSettlementModel->getProposalsByRespectedData($serviceType, $caseNo);
    ...
    public function vlbEncroacherDetails()
    {
        if (isset($_GET['dag']) && isset($_GET['m']) && isset($_GET['l']) && isset($_GET['v']) && isset($_GET['dist']) && isset($_GET['cir']) && isset($_GET['sub_div'])) {
            ...
            $vlb_encroacher = $this->OfflineSettlementModel->getEncroacherDetails($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_townprt_code, $dag_no);
            $vlb_enc['vlb_enc'] = $vlb_encroacher;
            if ($vlb_encroacher == true) {
                // getting the encroacher details
                $vlb_encroacher_in_dag = $this->OfflineSettlementModel->getEncroacherInDag($vlb_encroacher->id);
    ...
    public function getAllApplicationUnderProposalDept()
    {
        ...
        $pendingCase = $this->OfflineSettlementModel->getAllCasesUnderProposalDept($proposal_no, $dist_code);

        $remainingCase = $this->OfflineSettlementModel->getRemainingCasesUnderProposalDept($proposal_no, $dist_code);
    ...
    public function searchProposalIdByAppCaseNo()
    {
        ...
        if (trim($this->input->post('caseNo')) != null or trim($this->input->post('caseNo')) != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $countApplicationDetails = $this->OfflineSettlementModel->countApplicationDetailsByAppNo($app_no);
                ...
                $getApplicationDetails = $this->OfflineSettlementModel->getApplicationDetailsByAppNo($app_no);
        ...
        $countCaseIdSdlacProposal = $this->OfflineSettlementModel->countProposalIdByCaseNo($case_no);
        ...
        $getSdlacProposalID = $this->OfflineSettlementModel->getProposalIdByCaseNo($case_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($allSelectedList)) {
                ...
                foreach ($allSelectedList as $row) {
                    ...
                    if ($this->OfflineSettlementModel->updateSettlementBasicData($case_no, $updateData) <= 0) {
                    ...
                    if ($this->OfflineSettlementModel->updateProposalData($case_no, $updateProposalData) <= 0) {
                        $this->db2->trans_rollback();
                    ...
                    //////////////POST To basundhara/////////////////////
                    $application_no = $this->OfflineSettlementModel->getApplicationNoByCaseNo($this->db2,$case_no)->applid;
                    ...
                    $rtps_status = $this->OfflineSettlementModel->postApiBasundhara($rtpsno, $case, $rmk, $status, $task, $pen);
    ...
    public function getAllProposalListUnderSdlac()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->OfflineSettlementModel->getSdlacProposalsByService($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
                $data['proposalsCount'] = $proposals->num_rows();
                $data['_view'] = 'SettlementView/Sdlac/ProposalSearchPageSdlac';
                $this->load->view('layouts/main', $data);
    ...
    public function viewAllApplicationUnderProposalSdlac()
    {
        ...
        $pendingCase = $this->OfflineSettlementModel->getAllCasesUnderProposalSdlac($proposal_no, $dist_code);
    ...
    public function agreeProposalBySdlac()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($proposal_no != NULL && $dist_code != NULL && $service_code != NULL && $sdlac_user_code != NULL) {
                ...
                $updateSdlacMemberReportStatus = $this->OfflineSettlementModel->updateSettlementSdlacMemberReport($proposal_no, $dist_code, $service_code, $sdlac_user_code, $updateData);
                log_message('error', $this->db2->last_query());
                ...
                $updateProposalListQuery = $this->OfflineSettlementModel->updateSettlementProposalList($proposal_no, $dist_code, $service_code, $updateProposalList);
   ...
   public function loginSdlacWithoutAdhaar(){
    ...
    public function getAllSdlacProposalMeeting()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->OfflineSettlementModel->getSdlacProposalsMeetingByService($dist_code,$serviceType, $sdlac_user_code);
    ...
    public function searchCasesSdlac()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $circles = $this->OfflineSettlementModel->getcirclebyDistCode($dist_code);
    ...
    public function getCirCodeJson($dist_code)
    {
        ...
        $data = $this->OfflineSettlementModel->getCirCodeJSON($dist_code);
        $json = array();
    ...
    public function viewSdlacCaseLIst()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $results = $this->OfflineSettlementModel->viewSdlacSearchData($start, $length, $order, $searchByCol_0,$cir_code,$serviceType,$appStatus,$caseNo,$applicationNo,$pendingOffice,$fromDate,$toDate);
    ...
    public function getAllProposalListUnderSdlacOnlineOffline()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->OfflineSettlementModel->getSdlacProposalsByServiceOnlineOffline($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
    ...
    public function getMeetingListByDistrict()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $meetings = $this->OfflineSettlementModel->getMeetingsByDistrict();
    ...
    public function getAllProposalsUnderDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $proposal = $this->OfflineSettlementModel->getProposalsByMeeting($meeting_id);
    ...
    public function getAllCasesUnderProposalDept()
    {
        ...
        $pendingCase = $this->OfflineSettlementModel->getAllCasesUnderProposal($proposal_no);
    ...
    public function downloadCasesWithProposalId()
    {
        ...
        $this->OfflineSettlementModel->downloadExcelReport($file_name, $data);
    }
    ...
    public function checkListForAllotment()
    {
        ...
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $settlement['settlement_basic'] = $this->OfflineSettlementModel->getSettlementBasic($case_no);
            $settlement['settlement_enc'] = $this->OfflineSettlementModel->getSettlementEncroacher($case_no);
            $settlement['proceedings']   = $this->OfflineSettlementModel->getSettlementProceeding($case_no);
            $settlement['roadside_reservation']   = $this->OfflineSettlementModel->getSettlementRoadsideReservation($case_no);
            $settlement['family_reservation']   = $this->OfflineSettlementModel->getSettlementFamilyReservation($case_no);
            $settlement['settlement_applicant']  = $this->OfflineSettlementModel->getSettlementApplicant($case_no);
            ...
            $settlement['settlement_dag_area'] = $this->OfflineSettlementModel->getSettlementDagArea($case_no);
            $settlement['trace_map'] = $this->OfflineSettlementModel->getTraceMap($case_no);
            $settlement['field_report'] = $this->OfflineSettlementModel->getFieldReport($case_no);
            $settlement['geo_tag'] = $this->OfflineSettlementModel->getGeoTag($case_no);
            $settlement['settlement_ap_lmnote'] = $this->OfflineSettlementModel->getSettlementLmNote($case_no);
            $settlement['applicants_buyers']   = $this->OfflineSettlementModel->getAllApplicantBuyers($case_no);
    ...
    public function verifyCaseByAssistant()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $caseCount = $this->OfflineSettlementModel->countSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
            ...
            if ($caseCount == 0) {
                ...
                $this->db2->trans_begin();
                if ($this->OfflineSettlementModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData) == 0) {
                    $this->db2->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function viewAllCabinetCaseList()
    {
        ...
        $results = $this->OfflineSettlementModel->viewCabinetCaseList($start, $length, $order, $dist_code);
    ...
    public function downloadRecommendedCaseReportByDist()
    {
        ...
        if (!empty($allSelectedList)) {
            foreach ($allSelectedList as $row) {
                ...
                $settlement_basic = $this->OfflineSettlementModel->getSettlementBasic($case_no);

                $applicants   = $this->OfflineSettlementModel->getAllApplicantBuyersName($case_no)->result();
                ...
                $dags = $this->OfflineSettlementModel->getAllAppliedDags($case_no)->result();
                ...
                $sdlacStatus = $this->OfflineSettlementModel->sdlacCaseStatus($case_no)->row();

                $settlement_enc = $this->OfflineSettlementModel->getSettlementEncroacher($case_no);
                ...
                $landless = $this->OfflineSettlementModel->getLandLessVerify($case_no)->row();

                $geo_tag = $this->OfflineSettlementModel->getGeoTag($case_no);
        ...
        $this->OfflineSettlementModel->downloadExcelReport($file_name, $finalArray);
    }
    ...
    public function viewAllCasesUnderProposalDept()
    {
        ...
        $results = $this->OfflineSettlementModel->getAllCasesUnderProposal2($start, $length, $order, $dist_code);
    ...
    public function addCasesToCabMemo_Old()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($allSelectedList as $item) {
                ...
                $cabMemoCases = $this->OfflineSettlementModel->getExistingCasesUnderCabMemo($dist_code);
                ...
                if(CHITHA_AREA_VALIDATION == 1)
                {
                    ...
                    {
                        $checkArea = $this->OfflineSettlementModel->chithaReserveAreaCheckWithCaseNo($this->db2,$case); 
                ...
                $pullRequestCase = $this->OfflineSettlementModel->getPullRequestStatus($this->db2,$casesList);
                ...
                $user_code = $this->session->userdata('user_code');
                $this->db = $this->load->database('db2', TRUE);
                $memo_name = $this->OfflineSettlementModel->getMemoNameByCabId($this->db,$cabmemo_id);
                ...
                if (!empty($allSelectedList)) {
                    ...
                    foreach ($allSelectedList as $row) {
                        ...
                        if ($insertCabList != TRUE) {
                            ...
                            $updateCabIdList = $this->OfflineSettlementModel->updateCabStatus($this->db,$where, $updateData);
                            if ($updateCabIdList <= 0) {
                                ...
                                $updateBasicStatus = $this->OfflineSettlementModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);
    ...
    public function downloadReportForCabMemo()
    {
        ...
        $dist_codes = $this->OfflineSettlementModel->getDistrictUnderCabMemo($this->db,$cab_memo_id)->result();
        ...
        foreach ($dist_codes as $dist) {
            $allSelectedList = $this->OfflineSettlementModel->getCasesUnderCabMemo($this->db,$cab_memo_id, $dist)->result();
            ...
            if (!empty($allSelectedList)) {
                foreach ($allSelectedList as $row) {
                    ...
                    $settlement_basic = $this->OfflineSettlementModel->getSettlementBasic($case_no);
                    $caseDetails = $this->OfflineSettlementModel->getOfflineSdlacDetailsByCaseNo($case_no);
                    ...
                    $table2 .= '<tr>
                        ...
                        <td>' . $circle_name . '</td>
                        <td>' . $village_name . '</td>
                        <td>' . $caseDetails->house_type . '</td>
                        <td>' . $recData . '</td>
                        <td>' . date("j F, Y", strtotime($caseDetails->sdlac_rec_date)) . '</td>
                        <td>' . $caseDetails->nature_of_possession.'</td>
                        <td>' . $appGender .  '</td>
                        <td style="color:blue">' . $caseDetails->zonal_value .  '</td>
    ...
    public function clearCaseData(){
        ...
        foreach($district as $dist)
        {
            ...
            $allSelectedList = $this->OfflineSettlementModel->getCasesUnderDistCabMemo($district)->result();
            // Switch DB based on dist_code
            $this->db2 = $this->dbswitch2($district);
            ...
            if (!empty($allSelectedList)) {
                foreach ($allSelectedList as $row) {
                    ...
                    $update_basic = $this->OfflineSettlementModel->clearBasicData($where, $updateData);
                }
            }
        }
    ...
    $clearCabDetails = $this->OfflineSettlementModel->clearCabDetails();
  ...
  public function getAllCasesUnderDept() {
    ...
    $cases_list = $this->OfflineSettlementModel->getAllCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code,$meeting_no,$verification,$pullRequest);
    ...
    if(!empty($cases_list)) {
      ...
      if($cases_list['total_records'] > 0){
        ...
        foreach($data_rows as $row) {
            ...
            $minutelink = base_url() . "index.php/OfflineSettlement/viewMinute/". $dist_code . "/" . $row->meeting_id ;
          ...
          $link = base_url() . "index.php/OfflineSettlement/settlementBasu/?app=".$application_no . "&dist_code=" .$dist_code;
          $view_case = "<a href=".$link." class='btn btn-sm btn-primary' target='_blank'><i class='fa fa-eye'></i> &nbsp;Details</a>";

          $button = $view_case;
          
          $json[] = array(
            $row->case_no . '@' . $row->meeting_id . '@',
            $case_no .$deptRevertedStatus,
            '<small>' .$service .'</small>',
    ...
    $cases_list = $this->OfflineSettlementModel->getAllCasesUnderAssistant($this->db2,$start, $length, $order,$service_code,$meeting_no);
            ...
            $user_code = $this->session->userdata('user_code');
                ...
                if (!empty($allSelectedList)) {
                    ...
                    foreach ($allSelectedList as $row) {
                        ...
                        $this->db2->trans_begin();
                        $checkCasesSendStatus = $this->OfflineSettlementModel->checkCaseStatusVerify($case_no, $dist_code);
                        log_message('error',"999999999999---------".$checkCasesSendStatus);
                        if($checkCasesSendStatus != null){
                            $this->db2->trans_rollback();
                            log_message('error', '#OFFLINEMBERRSFV01: Already Verified in settlement_basic ');
                        ...
                        if($updateBasicStatus <= 0){
                            $this->db2->trans_rollback();
                            log_message('error', '#OFFLINEERRSFV: Updation failed in settlement_basic when send for verification');
                            log_message('error', $this->db2->last_query());
    ...
    public function getCasesListByDistrict()
    {
        if (($this->session->userdata('designation') == ASSISTANT_USERCODE) || ($this->session->userdata('designation') == UNDER_SEC_USERCODE)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'offlineSettlement/Dept/AllPendingCaseListUnderAssistant';
                $this->load->view('layouts/main', $data);
            } else {
    ...
    public function casesListForFinalApprovalByDept()
    {
        ...
        $user_code   = $this->session->userdata('user_code');
        $this->db = $this->load->database('db2', TRUE);
        $cases_list = $this->OfflineSettlementModel->getAllCasesFromMemoForFinalApproval($this->db,$cab_id,$user_code);

        $digitalSignedStatus = $this->OfflineSettlementModel->digitalSignedStatusOfCabId($this->db,$cab_id);
        ...
        $data['_view'] = 'SettlementView/Dept/caseListForFinalApproval';
        $this->load->view('layouts/main', $data);

    }


    public function casesListForFinalApprovalByDeptOffline()
    {
        $cab_id = $this->input->get('cab_id');
        // $dist_code   = $this->session->userdata('dist_code');
        $user_code   = $this->session->userdata('user_code');
        ...
        $data['finalCaseCount'] = $cases_list->num_rows();
        $data['digitalSignedStatus'] = $digitalSignedStatus->notification_digital_sign_status;
        $data['_view'] = 'offlineSettlement/Dept/caseListForFinalApprovalOffline';
        $this->load->view('layouts/main', $data);
    ...
    public function revertToDcByDeptTemp()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $updateBasicStatus = $this->OfflineSettlementModel->updateSettlementBasicData($case_no, $updateBasicData);
            ...
            //////proceeding start//////
                ...
                $insertProceeding = $this->db2->insert('settlement_proceeding', $insPetProceed);
                    ...
                    if ($insertProceeding != 1) {
                        ...
                        $updateProposalRevert = $this->OfflineSettlementModel->updateProposalData($case_no, $updateProposalData);
                                    ...
                                    // var_dump($rtps_status);
                                     ...
                                     if (trim($rtps_status) != 'y') {
                                        ...
                                        $this->db = $this->load->database('db2', TRUE);
                                            ...
                                            $updateCabMemoList = $this->OfflineSettlementModel->updateCabMemoList($updateData, $where);
    ...
    public function finalSubmissionByDept()
    {
            ...
            } else 
            {
                ...
                $this->db = $this->load->database('db2', TRUE);
                $dist_codes = $this->OfflineSettlementModel->getDistrictUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                if($errCheck == 0)
                {
                    ...
                    {
                        $this->db = $this->load->database('db2', TRUE);
                        $caseListForFinalSubmitByDist = $this->OfflineSettlementModel->getCaseListDetailsForFinalSubmit($this->db,$cabinet_id,$user_code,$dist->dist_code)->result();
                    ...
                    $affectedRows = $this->OfflineSettlementModel->updateCabStatus($this->db,$whereUpload, $updateUpload);
                    if($affectedRows <= 0){
    ...
    public function getRevertedCasesDetails()
    {
        ...
        $revertedCases = $this->OfflineSettlementModel->getAllRevertedCaseDetails($cabId);
    ...
    public function getAllRevertedCasesListByDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE || $this->session->userdata('designation') == DPT_JS) {
            ...
            $data['user_dist']      = $this->OfflineSettlementModel->getDeptUserDistListWithRevertedCaseCount();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'offlineSettlement/dept/AllRevertedCaseListUnderDpt';
                $this->load->view('layouts/main', $data);
            } else {
    ...
    $cases_list = $this->OfflineSettlementModel->getAllRevertedCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code);
    ...
    public function downloadRevertedCaseListReport()
    {
        ...
        foreach ($distMeeting as $dist) {
            ...
            $meetingCaseReverted = $this->OfflineSettlementModel->getRevertedCasesCountByDistMeeting($this->db2,$dist->meeting_id)->result();
    ...
    public function downloadRevertedCaseListReportOnlyRemark()
    {
        ...
        $currentCabCreationDate  = $cab_id_date->created_at;
            ...
            $deptCaseReverted = $this->OfflineSettlementModel->getRevertedCasesByDptBeforeCabApproval($this->db2,$cab_finalized_date, $cab_approve_date)->result();

            // $caseListNotUnderCabMemo = $this->OfflineSettlementModel->ListOfRevertCasesNotUnderCabMemo($this->db2,$dist->dist_code,$currentCabCreationDate,$cab_id_date->id);
    ...
    public function getRemarksDetailsRevertedCases()
    {
        ...
        $this->form_validation->set_rules('district_id', 'District ID', 'trim|required|is_natural');
         ...
         if ($this->form_validation->run() == FALSE) {
            ...
            $this->db2 =  $this->dbswitch2($dist_code);
            $revertedCases = $this->OfflineSettlementModel->getDetailsToBeRevertedCase($this->db2,$commaSeparatedCases);
            echo json_encode($revertedCases);
        }
    ...
    public function bulkApproveCasesByDPT()
    {
        ...
        } else
        {
                ...
                $caseWithDist = $this->OfflineSettlementModel->getDistrictCasesUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                if($errCheck == 0)
                {
                    ...
                    $updateCabMemoStatus = $this->OfflineSettlementModel->updateCabMemoList($updateCabMemoData, $whereMemo);
                    ...
                    $updateCabIdStatus = $this->OfflineSettlementModel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                    ...
                    else
                    {
                            ...
                            $task   = MB_DEPARTMENT;
                            $pen    = MB_CIRCLE_OFFICER;
                            // $rtps_status = $this->OfflineSettlementModel->applicationStatusUpdateBulk($concatenatedAppNo,'NA',$rmk,$status,$task,$pen);
                                            ...
                                            if($final_status == TEMP_APPROVE_BY_DEPT)
                                            {
                                                ...
                                                $updateProposalData[] = [
                                                    ...
                                                    $insPetProceed[] = [
                                                        ...
                                                        'date_of_hearing' => date('Y-m-d h:i:s'),
                                                        'next_date_of_hearing' => date('Y-m-d h:i:s'),
                                                        'note_on_order' => 'Approved by Department & send to CO',
                                                        'user_code' => $user_code,
                                                        'date_entry' => date('Y-m-d h:i:s'),
                                                        'operation' => 'E',
                                                        'ip' => $_SERVER['REMOTE_ADDR'],
                                        ...
                                        }else
                                        {
                                            ...
                                            $updateProposalData = $this->db2->update_batch('settlement_proposal_cases',$updateProposalData,'case_no');
                                                ...
                                                else
                                                {
                                                    ...
                                                    $cabIdUpdate = $this->OfflineSettlementModel->updateCabStatus($this->db,$whereCabID, $updateCabPartial);
                                                    if($cabIdUpdate <= 0){
                        ...
                        if (!empty($allSelectedList)) 
                            {
                                ...
                                if (!empty($duplicates)) 
                                {
                                    ...
                                    $this->db2 =  $this->dbswitch2($dist_code);
                                    $checkPullRequestFinalStatus = $this->OfflineSettlementModel->checkPullRequestFinalStatus($this->db2,$duplicateCases)->result();
                                        ...
                                        foreach($casesExist as $casesE)
                                            {
                                                ...
                                                $this->db = $this->load->database('db2', TRUE);
                                                $getOldCaseDetails = $this->OfflineSettlementModel->getOldCaseDetailsFromCabMemo($this->db,$caseNoOld,$dist_code)->row();

                                                $oldCaseNo = $getOldCaseDetails->case_no;

                                                $this->db2 =  $this->dbswitch2($dist_code);
                                                $getMeetingProposal = $this->OfflineSettlementModel->getMeetingProposalByCaseNo($this->db2,$caseNoOld)->row();
                                                    ...
                                                    $caseBackupDetails = [
                                                        ...
                                                        {
                                                            $updateCabMemoList = $this->OfflineSettlementModel->updateCabMemoList($newCaseDetails,$whereCase);
                                                            ...
                                                            if ($updateCabMemoList <= 0) {
                                                                ...
                                                                $updateCabIdListStatus = $this->OfflineSettlementModel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                                                                ...
                                                                return false;
                                                                    ...
                                                                    $updateBasicStatusOld = $this->OfflineSettlementModel->updateSettlementBasicForCab($this->db2,$oldCaseNo, $dist_code, $updateBasicData);
                                ...
                                if (!empty($newCases)) 
                                {
                                    foreach ($newCases as $caseN) 
                                    {
                                        ...
                                        $this->db2 =  $this->dbswitch2($dist_code);
                                        $getMeetingProposal = $this->OfflineSettlementModel->getMeetingProposalByCaseNo($this->db2,$case_no)->row();
                                        ...
                                        if ($insertCabList != TRUE) {
                                            ...
                                            if ($updateCabIdList <= 0) {
                                                $this->db->trans_rollback();
                                                log_message('error', '#OFFLINEERR6191: Updation failed in cab_id_list for bulk Approve');
                                                log_message('error', $this->db->last_query());

                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message' => '#OFFLINEERR6191: Failed to Add Cases to Cabinet Memo',
                                                ...
                                                $updateBasicStatusNew = $this->OfflineSettlementModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);


                                                if($updateBasicStatusNew <= 0)
                                                {
                                                    $this->db2->trans_rollback();
                                                    log_message('error', '#OFFLINEERRDUPDATBASICCAB: Updation failed in settlement_basic for change cabinet status');
                                                    log_message('error', $this->db2->last_query());

                                                    echo json_encode(array(
                                                        'responseType' => 1,
                                                        'message' => '#OFFLINEERRDUPDATBASICCAB: Failed to Add Cases to Cabinet Memo',
    ...
    public function getViewOfflineUploadedMinutes()
    {
        ...
        $fileDetails = $this->OfflineSettlementModel->getMinutesDocWithFileId($filePathId);
