<?php
class DeptReclassSuite extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Reclass_Suite/DeptReclassSuiteModel');
        $this->db2 = NULL;
    }
    ...
    public function landingPage()
    {
        ...
        elseif($designation  == ASSISTANT_USERCODE)
        {
            ...
            $data['_view'] = 'reclass_suite/landing_page_asst';
        }
        else
        {
            $data['verificationType'] = JS_VERIFICATION;
            $data['_view'] = 'reclass_suite/landing_page_joint_sec';
        }
        $this->load->view('layouts/main', $data);
    }

    public function viewPendingReclassSuiteCases()
    {
        ...
        $data['status'] = true;
        $this->load->view('reclass_suite/relcass_suite_case_list',$data);
	}
    
    public function getPendingReclassSuiteCaseList() 
    {
        $json = null;
        ...
        $case_list = $this->DeptReclassSuiteModel->getPendingCaseListDetails($this->db2,$start, $length, $order,$dist_code,$searchByCol_0);
        ...
        if($case_list['total_records'] >  0){
            ...
            foreach($data_rows as $row) {
                ...
                if($ast_verification == 'A'){
                    ...
                    $url = base_url('DeptReclassSuite/viewAssistantVerificationDetails');
                    $url .= '?dist_code=' . urlencode($row->dist_code);
                    $url .= '&case_no=' . urlencode($row->case_no);
    ...
    public function reclassSuiteCaseDetails()
    {
        $dist_code      = $this->input->get('dist_code');
        $application_no = $this->input->get('case_no');
        $this->db2      =  $this->dbswitch2($dist_code);

        $basic                 = $this->DeptReclassSuiteModel->getSettlementBasic($this->db2,$application_no);
        $applicants_buyers     = $this->DeptReclassSuiteModel->getAllApplicantBuyers($this->db2,$application_no);
        $applicants_owners     = $this->DeptReclassSuiteModel->getAllApplicantOwners($this->db2,$application_no);
        $applicants_encroacher = $this->DeptReclassSuiteModel->getAllApplicantEncroacher($this->db2,$application_no);
        $applicants_riotee_nok = $this->DeptReclassSuiteModel->getAllApplicantRioteeNok($this->db2,$application_no);
        $dags                  = $this->DeptReclassSuiteModel->getSettlementDag($this->db2,$application_no);
        $lmnotes               = $this->DeptReclassSuiteModel->getSettlementTenantLmNote($this->db2,$application_no);
        $proceedings           = $this->DeptReclassSuiteModel->getSettlementProceeding($this->db2,$application_no);
        $dhardocuments         = $this->DeptReclassSuiteModel->getDocuments($this->db2,$application_no);
        $nominee               = $this->DeptReclassSuiteModel->getAllNomineeDetail($this->db2,$application_no);
        ...
        $premiumData                   = $this->DeptReclassSuiteModel->getPremium($this->db2,$application_no);
        ...
        $data['deleted_dags']          = $this->DeptReclassSuiteModel->getDeletedDags($this->db2,$application_no);
        ...
        $caseDetails = $this->DeptReclassSuiteModel->getSettlementApplicationDetailsByCaseNo($this->db2,$case_no,$dist_code);
        $proceedings = $this->DeptReclassSuiteModel->getSettlementProceeding($this->db2,$case_no);
        //$data['caseCount']   = $caseCount;
        $data['caseDetails'] = $caseDetails;
        $data['proceedings'] = $proceedings;
        $data['reservation'] = $this->DeptReclassSuiteModel->getSettlementReservation($this->db2,$application_no);
        $data['additional_property'] = $this->DeptReclassSuiteModel->getAdditionalProperty($this->db2,$application_no);
        ...
        $data['instituteDetails'] = $this->DeptReclassSuiteModel->getInstitutionDetails($this->db2,$application_no);

        $data['_view'] = 'reclass_suite/case_view';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function sentReclassSuiteCasesForVerification()
    {
            ...
            if (!empty($allSelectedList)) 
                {
                    foreach ($allSelectedList as $caseN) 
                    {
                        ...
                        $updateSetlmntBasicStatus = $this->DeptReclassSuiteModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData,$insSetProceed);
    ...
    public function viewReclassSuiteCasesDptAsst()
    {
        $dist_code   = $this->input->post('dist_code');
        $data = array();
        $data['dist_code'] =$dist_code;
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $data['status'] = true;
        $this->load->view('reclass_suite/case_list_pending_under_asst',$data);
	}

    public function getPendingReclassSuiteCaseListUnderAssistant() 
    {
        ...
        $case_list = $this->DeptReclassSuiteModel->getPendingCaseListDetailsForVerificationAsst($this->db2,$start, $length, $order,$dist_code,$searchByCol_0);
    ...
    public function sentReclassSuiteCasesFromAsoToJs()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
                ...
                if ($case_no != null || $case_no!= "") 
                    {
                        ...
                        if($updateSetlmntBasicStatus == 'SERVER-ERROR'){
                        $this->db2->trans_rollback();
                        log_message('error', '#ERR711: Updation failed in reclass_suite_basic and proceeding for change JS Verification Status');
                        log_message('error', $this->db2->last_query());
                        echo json_encode(array(
                            'responseType' => 1,
    ...
    public function viewAssistantVerificationDetails()
    {
        ...
        $this->db = $this->dbswitch2($dist_code);
        $assistantVerification=$this->DeptReclassSuiteModel->getAssistantVerificationDetails($this->db,$case_no)->result();
