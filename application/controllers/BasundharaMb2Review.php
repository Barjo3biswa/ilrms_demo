<?php
class BasundharaMb2Review extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('basundhara/basundharaMb2Reviewmodel');
        $this->db2 = NULL;
    }
    ...
    function request($service)
    {
        ...
        $settlement['pending'] = $this->basundharaMb2Reviewmodel->getDepartmentRequest($service, $dist_list);
    ...
    function deptApprovedCases($service)
    {
        //var_dump($_SESSION);
        $this->load->model('basundhara/basundharaMb2Reviewmodel');
        $settlement['pending'] = $this->basundharaMb2Reviewmodel->alldepartmentApprovedCases($service);
    ...
    function settlementBasu()
    {
        ...
        $settlement['settlement_basic'] = $this->basundharaMb2Reviewmodel->getSettlementBasic($case_no);
        $settlement['settlement_applicant']  = $this->basundharaMb2Reviewmodel->getSettlementApplicant($case_no);
        //var_dump($settlement['settlement_applicant']);die();
        $settlement['settlement_dag_details'] = $this->basundharaMb2Reviewmodel->getSettlementDagDetails($case_no);
        $settlement['settlement_dag_area'] = $this->basundharaMb2Reviewmodel->getSettlementDagArea($case_no);
        $settlement['settlement_proceeding'] = $this->basundharaMb2Reviewmodel->getSettlementProceeding($case_no);
        $settlement['settlement_ap_lmnote'] = $this->basundharaMb2Reviewmodel->getSettlementLmNote($case_no);
        $settlement['supportive_document'] = $this->basundharaMb2Reviewmodel->getSupportiveDocuments($case_no);
        $settlement['proceedings']   = $this->basundharaMb2Reviewmodel->getSettlementProceeding($case_no);
        // Newly added on 08/09/2022
        $settlement['applicants_buyers']   = $this->basundharaMb2Reviewmodel->getAllApplicantBuyers($case_no);
        $settlement['applicants_owners']   = $this->basundharaMb2Reviewmodel->getAllApplicantOwners($case_no);
        $settlement['applicants_encroacher']   = $this->basundharaMb2Reviewmodel->getAllApplicantEncroacher($case_no);
        $settlement['applicants_riotee_nok']   = $this->basundharaMb2Reviewmodel->getAllApplicantRioteeNok($case_no);

        // New Added Settlemet Reservation Details
        $settlement['roadside_reservation']   = $this->basundharaMb2Reviewmodel->getSettlementRoadsideReservation($case_no);
        $settlement['family_reservation']   = $this->basundharaMb2Reviewmodel->getSettlementFamilyReservation($case_no);
        $settlement['vgr_reservation']   = $this->basundharaMb2Reviewmodel->getSettlementVgrReservation($case_no);

        // Premium Calculation Details
        if($settlement['settlement_basic']["service_code"] != SETTLEMENT_SPECIAL_CULTIVATORS_ID)
        {
            $this->db2->trans_begin();
            $settlement_premium_insertion = $this->basundharaMb2Reviewmodel->premiumReCalculation($this->db2,$case_no);
        ...
        {
            $this->db2->trans_begin();
            $settlement_premium_insertion = $this->basundharaMb2Reviewmodel->premiumReCalculationTea($this->db2,$case_no);
        ...
        $settlement['premium_data']  = $this->basundharaMb2Reviewmodel->getSettlementPremium($case_no);
        $settlement['landmark_data'] = $this->basundharaMb2Reviewmodel->getSettlementDagArea($case_no);
        $settlement['possession_data']   = $this->basundharaMb2Reviewmodel->getAllPossessionDetails($case_no);


        //*******getting the deleted settlement_dag_details data from settlement_deleted_data table */
        $deletedEnc=$this->basundharaMb2Reviewmodel->getDeletedEncroacher($case_no);
        ...
        $deletedDags=$this->basundharaMb2Reviewmodel->getDeletedDags($case_no);
        $deletedData = array();
        ...
        $getJsonBackup = $this->basundharaMb2Reviewmodel->getJsonDataFromBackup($case_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($dist_code == '' and $caseNo == ''  and $applicationNo == '' and $serviceType == '' and $appStatus == '' and $pendingOffice == '' and $fromDate == '' and $toDate == '' and $cir_code == '') {
                ...
                if ($caseNo != '') {
                    ...
                    $cases = $this->basundharaMb2Reviewmodel->getCasesByCaseNo($caseNo);
                    ...
                    $cases = $this->basundharaMb2Reviewmodel->getCasesByApplicationNo($applicationNo);
                    $data['cases']      = $cases->result();
                    $data['casesCount'] = $cases->num_rows();
                    ...
                    if ($fromDate != '' and $toDate != '') {
                        ...
                        $cases = $this->basundharaMb2Reviewmodel->getCasesByRespectedDataWithDateRage($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
                        ...
                        $cases = $this->basundharaMb2Reviewmodel->getCasesByRespectedData($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
    ...
    public function searchProposalWIthData()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->basundharaMb2Reviewmodel->getProposalsByRespectedData($serviceType, $caseNo);
    ...
    public function vlbEncroacherDetails()
    {
        if (isset($_GET['dag']) && isset($_GET['m']) && isset($_GET['l']) && isset($_GET['v']) && isset($_GET['dist']) && isset($_GET['cir']) && isset($_GET['sub_div'])) {
            ...
            $vlb_encroacher = $this->basundharaMb2Reviewmodel->getEncroacherDetails($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_townprt_code, $dag_no);
            $vlb_enc['vlb_enc'] = $vlb_encroacher;
            if ($vlb_encroacher == true) {
                // getting the encroacher details
                $vlb_encroacher_in_dag = $this->basundharaMb2Reviewmodel->getEncroacherInDag($vlb_encroacher->id);
    ...
    public function getAllApplicationUnderProposalDept()
    {
        ...
        $pendingCase = $this->basundharaMb2Reviewmodel->getAllCasesUnderProposalDept($proposal_no, $dist_code);

        $remainingCase = $this->basundharaMb2Reviewmodel->getRemainingCasesUnderProposalDept($proposal_no, $dist_code);
    ...
    public function searchProposalIdByAppCaseNo()
    {
        ...
        if (trim($this->input->post('caseNo')) != null or trim($this->input->post('caseNo')) != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $countApplicationDetails = $this->basundharaMb2Reviewmodel->countApplicationDetailsByAppNo($app_no);
                ...
                $getApplicationDetails = $this->basundharaMb2Reviewmodel->getApplicationDetailsByAppNo($app_no);
        ...
        $countCaseIdSdlacProposal = $this->basundharaMb2Reviewmodel->countProposalIdByCaseNo($case_no);
        ...
        $getSdlacProposalID = $this->basundharaMb2Reviewmodel->getProposalIdByCaseNo($case_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($allSelectedList)) {
                ...
                foreach ($allSelectedList as $row) {
                    ...
                    if ($this->basundharaMb2Reviewmodel->updateSettlementBasicData($case_no, $updateData) <= 0) {
                        $this->db2->trans_rollback();
                    ...
                    if ($this->basundharaMb2Reviewmodel->updateProposalData($case_no, $updateProposalData) <= 0) {
                        $this->db2->trans_rollback();
                    ...
                    //////////////POST To basundhara/////////////////////
                    $application_no = $this->basundharaMb2Reviewmodel->getApplicationNoByCaseNo($this->db2,$case_no)->applid;
                    ...
                    $rtps_status = $this->basundharaMb2Reviewmodel->postApiBasundhara($rtpsno, $case, $rmk, $status, $task, $pen);
    ...
    public function getAllProposalListUnderSdlac()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->basundharaMb2Reviewmodel->getSdlacProposalsByService($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
                $data['proposalsCount'] = $proposals->num_rows();
                $data['_view'] = 'SettlementView/Sdlac/ProposalSearchPageSdlac';
                $this->load->view('layouts/main', $data);
    ...
    public function viewAllApplicationUnderProposalSdlac()
    {
        ...
        $pendingCase = $this->basundharaMb2Reviewmodel->getAllCasesUnderProposalSdlac($proposal_no, $dist_code);
    ...
    public function agreeProposalBySdlac()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($proposal_no != NULL && $dist_code != NULL && $service_code != NULL && $sdlac_user_code != NULL) {
                ...
                $updateSdlacMemberReportStatus = $this->basundharaMb2Reviewmodel->updateSettlementSdlacMemberReport($proposal_no, $dist_code, $service_code, $sdlac_user_code, $updateData);
                log_message('error', $this->db2->last_query());
                ...
                $updateProposalListQuery = $this->basundharaMb2Reviewmodel->updateSettlementProposalList($proposal_no, $dist_code, $service_code, $updateProposalList);
   ...
   public function loginSdlacWithoutAdhaar(){
    ...
    public function getAllSdlacProposalMeeting()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->basundharaMb2Reviewmodel->getSdlacProposalsMeetingByService($dist_code,$serviceType, $sdlac_user_code);
    ...
    public function searchCasesSdlac()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $circles = $this->basundharaMb2Reviewmodel->getcirclebyDistCode($dist_code);
    ...
    public function getCirCodeJson($dist_code)
    {
        ...
        $data = $this->basundharaMb2Reviewmodel->getCirCodeJSON($dist_code);
        $json = array();
    ...
    public function viewSdlacCaseLIst()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $results = $this->basundharaMb2Reviewmodel->viewSdlacSearchData($start, $length, $order, $searchByCol_0,$cir_code,$serviceType,$appStatus,$caseNo,$applicationNo,$pendingOffice,$fromDate,$toDate);
    ...
    public function getAllProposalListUnderSdlacOnlineOffline()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->basundharaMb2Reviewmodel->getSdlacProposalsByServiceOnlineOffline($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
    ...
    public function getMeetingListByDistrict()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $meetings = $this->basundharaMb2Reviewmodel->getMeetingsByDistrict();
    ...
    public function getAllProposalsUnderDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $proposal = $this->basundharaMb2Reviewmodel->getProposalsByMeeting($meeting_id);
    ...
    public function getAllCasesUnderProposalDept()
    {
        ...
        $pendingCase = $this->basundharaMb2Reviewmodel->getAllCasesUnderProposal($proposal_no);
    ...
    public function downloadCasesWithProposalId()
    {
        ...
        $this->basundharaMb2Reviewmodel->downloadExcelReport($file_name, $data);
    }
    ...
    public function checkListForAllotment()
    {
        ...
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $settlement['settlement_basic'] = $this->basundharaMb2Reviewmodel->getSettlementBasic($case_no);
            $settlement['settlement_enc'] = $this->basundharaMb2Reviewmodel->getSettlementEncroacher($case_no);
            $settlement['proceedings']   = $this->basundharaMb2Reviewmodel->getSettlementProceeding($case_no);
            $settlement['roadside_reservation']   = $this->basundharaMb2Reviewmodel->getSettlementRoadsideReservation($case_no);
            $settlement['family_reservation']   = $this->basundharaMb2Reviewmodel->getSettlementFamilyReservation($case_no);
            $settlement['settlement_applicant']  = $this->basundharaMb2Reviewmodel->getSettlementApplicant($case_no);
            ...
            $settlement['settlement_dag_area'] = $this->basundharaMb2Reviewmodel->getSettlementDagArea($case_no);
            $settlement['trace_map'] = $this->basundharaMb2Reviewmodel->getTraceMap($case_no);
            $settlement['field_report'] = $this->basundharaMb2Reviewmodel->getFieldReport($case_no);
            $settlement['geo_tag'] = $this->basundharaMb2Reviewmodel->getGeoTag($case_no);
            $settlement['settlement_ap_lmnote'] = $this->basundharaMb2Reviewmodel->getSettlementLmNote($case_no);
            $settlement['applicants_buyers']   = $this->basundharaMb2Reviewmodel->getAllApplicantBuyers($case_no);
    ...
    public function verifyCaseByAssistant()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $caseCount = $this->basundharaMb2Reviewmodel->countSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
            ...
            if ($caseCount == 0) {
                ...
                $this->db2->trans_begin();
                if ($this->basundharaMb2Reviewmodel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData) == 0) {
                    $this->db2->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function viewAllCabinetCaseList()
    {
        ...
        $results = $this->basundharaMb2Reviewmodel->viewCabinetCaseList($start, $length, $order, $dist_code);
    ...
    public function downloadRecommendedCaseReportByDist()
    {
        ...
        if (!empty($allSelectedList)) {
            foreach ($allSelectedList as $row) {
                ...
                $settlement_basic = $this->basundharaMb2Reviewmodel->getSettlementBasic($case_no);

                $applicants   = $this->basundharaMb2Reviewmodel->getAllApplicantBuyersName($case_no)->result();
                ...
                $dags = $this->basundharaMb2Reviewmodel->getAllAppliedDags($case_no)->result();
                ...
                $sdlacStatus = $this->basundharaMb2Reviewmodel->sdlacCaseStatus($case_no)->row();

                $settlement_enc = $this->basundharaMb2Reviewmodel->getSettlementEncroacher($case_no);
                ...
                $landless = $this->basundharaMb2Reviewmodel->getLandLessVerify($case_no)->row();

                $geo_tag = $this->basundharaMb2Reviewmodel->getGeoTag($case_no);
        ...
        $this->basundharaMb2Reviewmodel->downloadExcelReport($file_name, $finalArray);
    }
    ...
    public function viewAllCasesUnderProposalDept()
    {
        ...
        $results = $this->basundharaMb2Reviewmodel->getAllCasesUnderProposal2($start, $length, $order, $dist_code);
    ...
    public function getAllCasesListByDept()
    {
        if ($this->session->userdata('designation') == DPT_JS) {
            ...
            $data['user_dist']      = $this->basundharaMb2Reviewmodel->getDeptUserDistListWithCaseCount();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'SettlementView/Dept/AllPendingCaseListUnderDptReview';
                $this->load->view('layouts/main', $data);
            } else {

                $this->db2 =  $this->dbswitch2($dist_code);
                $meeting = $this->basundharaMb2Reviewmodel->getMeetingListByDist($this->db2,$dist_code);
                ...
                log_message('error','--'.$this->db->last_query());
                $cabIdList = $this->basundharaMb2Reviewmodel->getCabinetIdList($dist_code,$user_code)->result();
    ...
    public function addCasesToCabMemo_Old()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($allSelectedList as $item) {
                ...
                $cabMemoCases = $this->basundharaMb2Reviewmodel->getExistingCasesUnderCabMemo($dist_code);
                ...
                if(CHITHA_AREA_VALIDATION == 1)
                {
                    ...
                    {
                        $checkArea = $this->basundharaMb2Reviewmodel->chithaReserveAreaCheckWithCaseNo($this->db2,$case); 
                ...
                $pullRequestCase = $this->basundharaMb2Reviewmodel->getPullRequestStatus($this->db2,$casesList);
                ...
                $user_code = $this->session->userdata('user_code');
                $this->db = $this->load->database('db2', TRUE);
                $memo_name = $this->basundharaMb2Reviewmodel->getMemoNameByCabId($this->db,$cabmemo_id);
                ...
                if (!empty($allSelectedList)) {
                    ...
                    foreach ($allSelectedList as $row) {
                        ...
                        if ($insertCabList != TRUE) {
                            ...
                            $updateCabIdList = $this->basundharaMb2Reviewmodel->updateCabStatus($this->db,$where, $updateData);
                            if ($updateCabIdList <= 0) {
                                ...
                                $updateBasicStatus = $this->basundharaMb2Reviewmodel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);
    ...
    public function downloadReportForCabMemo()
    {
        ...
        $dist_codes = $this->basundharaMb2Reviewmodel->getDistrictUnderCabMemo($this->db,$cab_memo_id)->result();
        ...
        foreach ($dist_codes as $dist) {
            $allSelectedList = $this->basundharaMb2Reviewmodel->getCasesUnderCabMemo($this->db,$cab_memo_id, $dist)->result();
    ...
    public function clearCaseData(){
        ...
        foreach($district as $dist)
        {
            ...
            $allSelectedList = $this->basundharaMb2Reviewmodel->getCasesUnderDistCabMemo($district)->result();
            // Switch DB based on dist_code
            $this->db2 = $this->dbswitch2($district);
            ...
            if (!empty($allSelectedList)) {
                foreach ($allSelectedList as $row) {
                    ...
                    $update_basic = $this->basundharaMb2Reviewmodel->clearBasicData($where, $updateData);
                }
            }
        }
    ...
    $clearCabDetails = $this->basundharaMb2Reviewmodel->clearCabDetails();
  ...
  public function getAllCasesUnderDept() {
    ...
    $cases_list = $this->basundharaMb2Reviewmodel->getAllCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code,$meeting_no,$verification,$pullRequest);
    ...
    $cases_list = $this->basundharaMb2Reviewmodel->getAllCasesUnderAssistant($this->db2,$start, $length, $order,$service_code,$meeting_no);
            ...
            $user_code = $this->session->userdata('user_code');
                ...
                if (!empty($allSelectedList)) {
                    ...
                    foreach ($allSelectedList as $row) {
                        ...
                        $this->db2->trans_begin();
                        $checkCasesSendStatus = $this->basundharaMb2Reviewmodel->checkCaseStatusVerify($case_no, $dist_code);
    ...
    public function getCasesListByDistrict()
    {
        if (($this->session->userdata('designation') == ASSISTANT_USERCODE) || ($this->session->userdata('designation') == UNDER_SEC_USERCODE)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'SettlementView/Dept/AllPendingCaseListUnderAssistantReview';
                $this->load->view('layouts/main', $data);
            } else {
    ...
    public function casesListForFinalApprovalByDept()
    {
        ...
        $user_code   = $this->session->userdata('user_code');
        $this->db = $this->load->database('db2', TRUE);
        $cases_list = $this->basundharaMb2Reviewmodel->getAllCasesFromMemoForFinalApproval($this->db,$cab_id,$user_code);

        $digitalSignedStatus = $this->basundharaMb2Reviewmodel->digitalSignedStatusOfCabId($this->db,$cab_id);
        // var_dump($digitalSignedStatus );exit;
    ...
    public function revertToDcByDeptTemp()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $updateBasicStatus = $this->basundharaMb2Reviewmodel->updateSettlementBasicData($case_no, $updateBasicData);
            ...
            //////proceeding start//////
                ...
                $insertProceeding = $this->db2->insert('settlement_proceeding', $insPetProceed);
                    ...
                    if ($insertProceeding != 1) {
                        ...
                        $updateProposalRevert = $this->basundharaMb2Reviewmodel->updateProposalData($case_no, $updateProposalData);
                                    ...
                                    // var_dump($rtps_status);
                                     ...
                                     if (trim($rtps_status) != 'y') {
                                        ...
                                        $this->db = $this->load->database('db2', TRUE);
                                            ...
                                            $updateCabMemoList = $this->basundharaMb2Reviewmodel->updateCabMemoList($updateData, $where);
    ...
    public function finalSubmissionByDept()
    {
            ...
            } else 
            {
                ...
                $this->db = $this->load->database('db2', TRUE);
                $dist_codes = $this->basundharaMb2Reviewmodel->getDistrictUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                foreach($distMeeting as $dist)
                {
                    ...
                    $meetingCase = $this->basundharaMb2Reviewmodel->getCasesCountByDistMeeting($this->db2,$dist->meeting_id);

                    $pullRequestCase = $this->basundharaMb2Reviewmodel->getCasesHavingPullRequest($this->db2,$dist->meeting_id);
                ...
                if($errCheck == 0)
                {
                    ...
                    {
                        $this->db = $this->load->database('db2', TRUE);
                        $caseListForFinalSubmitByDist = $this->basundharaMb2Reviewmodel->getCaseListDetailsForFinalSubmit($this->db,$cabinet_id,$user_code,$dist->dist_code)->result();
                    ...
                    $affectedRows = $this->basundharaMb2Reviewmodel->updateCabStatus($this->db,$whereUpload, $updateUpload);
                    if($affectedRows <= 0){
    ...
    public function getRevertedCasesDetails()
    {
        ...
        $revertedCases = $this->basundharaMb2Reviewmodel->getAllRevertedCaseDetails($cabId);
    ...
    public function getAllRevertedCasesListByDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $data['user_dist']      = $this->basundharaMb2Reviewmodel->getDeptUserDistListWithRevertedCaseCount();
    ...
    $cases_list = $this->basundharaMb2Reviewmodel->getAllRevertedCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code);
    ...
    public function downloadRevertedCaseListReport()
    {
        ...
        foreach ($distMeeting as $dist) {
            ...
            $meetingCaseReverted = $this->basundharaMb2Reviewmodel->getRevertedCasesCountByDistMeeting($this->db2,$dist->meeting_id)->result();
    ...
    public function downloadRevertedCaseListReportOnlyRemark()
    {
        ...
        $currentCabCreationDate  = $cab_id_date->created_at;
            ...
            $deptCaseReverted = $this->basundharaMb2Reviewmodel->getRevertedCasesByDptBeforeCabApproval($this->db2,$cab_finalized_date, $cab_approve_date)->result();

            // $caseListNotUnderCabMemo = $this->basundharaMb2Reviewmodel->ListOfRevertCasesNotUnderCabMemo($this->db2,$dist->dist_code,$currentCabCreationDate,$cab_id_date->id);
    ...
    public function getRemarksDetailsRevertedCases()
    {
        ...
        $this->form_validation->set_rules('district_id', 'District ID', 'trim|required|is_natural');
         ...
         if ($this->form_validation->run() == FALSE) {
            ...
            $this->db2 =  $this->dbswitch2($dist_code);
            $revertedCases = $this->basundharaMb2Reviewmodel->getDetailsToBeRevertedCase($this->db2,$commaSeparatedCases);
            echo json_encode($revertedCases);
        }
                ...
                }else
                {
                        ...
                        }else
                        {
                            ...
                            }else
                            {
                                ...
                                $task   = MB_DEPARTMENT;
                                $pen    = MB_DEPUTY_COMM;
                                $rtps_status = $this->basundharaMb2Reviewmodel->applicationStatusUpdateBulk($concatenatedAppNo,'NA',$rmk,$status,$task,$pen);
    ...
    public function bulkApproveCasesByDPT()
    {
        ...
        } else
        {
                ...
                $caseWithDist = $this->basundharaMb2Reviewmodel->getDistrictCasesUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                if($errCheck == 0)
                {
                    ...
                    $updateCabMemoStatus = $this->basundharaMb2Reviewmodel->updateCabMemoList($updateCabMemoData, $whereMemo);
                    ...
                    $updateCabIdData = array(
                        ...
                        'updated_at'        => date('Y-m-d h:i:s'),
                        'approved_at'       => date('Y-m-d h:i:s'),
                        //'notification_digital_sign_status'  => NOTIFICATION_DIGITAL_SIGN_FINAL_STATUS,                       
                    );
                    ...
                    $updateCabIdStatus = $this->basundharaMb2Reviewmodel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                                        ...
                                        }else
                                        {
                                            ...
                                            $updateProposalData = $this->db2->update_batch('settlement_proposal_cases',$updateProposalData,'case_no');
                                                ...
                                                else
                                                {
                                                    ...
                                                    $cabIdUpdate = $this->basundharaMb2Reviewmodel->updateCabStatus($this->db,$whereCabID, $updateCabPartial);
                                                    if($cabIdUpdate <= 0){
    ...
    public function getAllVgrCicleDistWise()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $data['user_dist']      = $this->basundharaMb2Reviewmodel->getDeptUserDistListWithCaseCountVGR();
    ...
    public function getAllVGRCirclesByDist() 
    {
        ...
        $cases_list = $this->basundharaMb2Reviewmodel->getAllVGRCasesCircleByDist($this->db2,$start, $length, $order);
    ...
    public function getAllVGRCaseListByCircle()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $user_code = $this->session->userdata('user_code');
            $this->db2 =  $this->dbswitch2($dist_code);
            $meeting = $this->basundharaMb2Reviewmodel->getMeetingListByDistVGR($this->db2,$dist_code);
            $data['meetingList']      = $meeting->result();
            ...
            $data['ast_list'] = $this->db->query("SELECT name,user_code,email,designation FROM depart_users WHERE designation in ('$ast','$sec')",array())->result();
            log_message('error','--'.$this->db->last_query());
            $cabIdListVGR = $this->basundharaMb2Reviewmodel->getVGRCabinetIdList($dist_code,$user_code)->result();
    ...
    public function getAllVGRCasesByCircle() 
    {
        ...
        $cases_list = $this->basundharaMb2Reviewmodel->getAllVGRCasesByCircle($this->db2,$start, $length, $order,$subdiv_code,$cir_code,$meeting_no,$verification,$pullRequest);
    ...
    public function addVGRCasesToCabMemo_old()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($allSelectedList as $item) {
                ...
                $vgrRevertedCases = $this->basundharaMb2Reviewmodel->getRevertedCasesDetailsVGR($this->db2,$concatedCasesNo);
                ...
                //////////////VGR NEW CHITHA_AREA VALIDATION START////////////////
                    ...
                    $vgrReservationCases = $this->basundharaMb2Reviewmodel->getSettlementVgrReservationDetails($this->db2,$concatedCasesNo);
                    ...
                    else
                    {
                        ...
                        foreach($vgrReservationCasesList as $vgrReservation)
                        {
                            ...
                            $checkReserv = $this->basundharaMb2Reviewmodel->getTotalVgrReservationInDag($this->db2,$vgrReservation['dist_code'], $vgrReservation['subdiv_code'], $vgrReservation['cir_code'], $vgrReservation['mouza_pargona_code'], $vgrReservation['lot_no'], $vgrReservation['vill_townprt_code'], $vgrReservation['dag_no']);
                ...
                if(VGR_CLUSTER_VALIDATION == 1)
                    {
                        ...
                        {
                            $case_no = trim($case);
                            $CheckClusterCount = $this->basundharaMb2Reviewmodel->checkIfCaseRevertedFromCluster($this->db2,$case_no); 
    ...
    public function downloadRevertedCaseListReportVGR()
    {
        ...
        $previousCabApprovedDate  = $cab_id_date->updated_at;
            ...
            $deptCaseReverted = $this->basundharaMb2Reviewmodel->getVGRRevertedCasesByDptBeforeCabApproval($this->db2,$cab_finalized_date, $cab_approve_date)->result();

            // $caseListNotUnderCabMemo = $this->basundharaMb2Reviewmodel->ListOfVGRRevertCasesNotUnderCabMemo($this->db2,$dist->dist_code,$previousCabApprovedDate,$cab_id_date->id);
    ...
    public function casesListForFinalApprovalByDeptVGR()
    {
        ...
        $user_code   = $this->session->userdata('user_code');
        $this->db = $this->load->database('db2', TRUE);
        $cases_list = $this->basundharaMb2Reviewmodel->getAllVGRCasesFromMemoForFinalApproval($this->db,$cab_id,$user_code);
    ...
    public function getAllRevertedCasesListByDeptVGR()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $data['user_dist']      = $this->basundharaMb2Reviewmodel->getDeptUserDistListWithRevertedCaseCountVGR();
    ...
    public function getAllRevertedCasesUnderDeptVGR() 
    {
        ...
        $cases_list = $this->basundharaMb2Reviewmodel->getAllRevertedCasesUnderDepartmentVGR($this->db2,$start, $length, $order,$service_code);
        ...
        } else
        {
                ...
                if(VGR_CLUSTER_VALIDATION == 1)
                {
                    ...
                    foreach($caseWithDist as $caseDist)
                    {
                        ...
                        $CheckClusterCount = $this->basundharaMb2Reviewmodel->checkIfCaseRevertedFromCluster($this->db2,$vgr_case_no); 
                ...
                foreach($distMeeting as $dist)
                {
                    ...
                    $meetingCaseRevert = $this->basundharaMb2Reviewmodel->getMeetingCasesExistInVgrRevert($this->db2,$dist->meeting_id);
                    ...
                    {
                        $memoCase = $this->basundharaMb2Reviewmodel->getCasesCountFromCabMemo($dist->dist_code,$dist->meeting_id);
                                ...
                                foreach($dist_codes as $dist)
                                {
                                    ...
                                    if (!empty($caseListForFinalSubmitByDist)) 
                                    {
                                        ...
                                        foreach ($caseListForFinalSubmitByDist as $row) 
                                        {
                                            ...
                                            $clusterId = $this->basundharaMb2Reviewmodel->getClusterIdByCaseNo($this->db2,$case_no);
        ...
        } else 
        {
            ...
            if($errCheck == 0)
            {
                ...
                foreach($casesArray as $row)
                {
                    ...
                    $getBasicData = $this->basundharaMb2Reviewmodel->getBasicCasesDetailsVGR($this->db2,$case_no)->row();
                        ...
                        if (!empty($allSelectedList)) 
                            {
                                ...
                                if (!empty($duplicates)) 
                                {
                                    ...
                                    $this->db2 =  $this->dbswitch2($dist_code);
                                    $checkPullRequestFinalStatus = $this->basundharaMb2Reviewmodel->checkPullRequestFinalStatus($this->db2,$duplicateCases)->result();
                                        ...
                                        foreach($casesExist as $casesE)
                                            {
                                                ...
                                                $this->db = $this->load->database('db2', TRUE);
                                                $getOldCaseDetails = $this->basundharaMb2Reviewmodel->getOldCaseDetailsFromCabMemo($this->db,$caseNoOld,$dist_code)->row();

                                                $oldCaseNo = $getOldCaseDetails->case_no;

                                                $this->db2 =  $this->dbswitch2($dist_code);
                                                $getMeetingProposal = $this->basundharaMb2Reviewmodel->getMeetingProposalByCaseNo($this->db2,$caseNoOld)->row();
                                                    ...
                                                    $caseBackupDetails = [
                                                        ...
                                                        {
                                                            $updateCabMemoList = $this->basundharaMb2Reviewmodel->updateCabMemoList($newCaseDetails,$whereCase);
                                                            ...
                                                            if ($updateCabMemoList <= 0) {
                                                                ...
                                                                $updateCabIdListStatus = $this->basundharaMb2Reviewmodel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                                                                ...
                                                                return false;
                                                                    ...
                                                                    $updateBasicStatusOld = $this->basundharaMb2Reviewmodel->updateSettlementBasicForCab($this->db2,$oldCaseNo, $dist_code, $updateBasicData);
                                ...
                                if (!empty($newCases)) 
                                {
                                    foreach ($newCases as $caseN) 
                                    {
                                        ...
                                        $this->db2 =  $this->dbswitch2($dist_code);
                                        $getMeetingProposal = $this->basundharaMb2Reviewmodel->getMeetingProposalByCaseNo($this->db2,$case_no)->row();
                                        ...
                                        if ($insertCabList != TRUE) {
                                            ...
                                            if ($updateCabIdList <= 0) {
                                                ...
                                                $updateBasicStatusNew = $this->basundharaMb2Reviewmodel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);
    ...
    public function pendingReviewCaselanding()
    {
        if ($this->session->userdata('designation') == DPT_JS) {
