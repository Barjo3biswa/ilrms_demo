<?php
class Bhoodan extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('basundhara/BhoodanModel');
        $this->db2 = NULL;
    }
    ...
    function request($service)
    {
        ...
        $settlement['pending'] = $this->BhoodanModel->getDepartmentRequest($service, $dist_list);
    ...
    function deptApprovedCases($service)
    {
        //var_dump($_SESSION);
        $this->load->model('basundhara/BhoodanModel');
        $settlement['pending'] = $this->BhoodanModel->alldepartmentApprovedCases($service);
    ...
    function settlementBasu()
    {
        ...
        $settlement['settlement_basic'] = $this->BhoodanModel->getSettlementBasic($case_no);
        $settlement['settlement_applicant']  = $this->BhoodanModel->getSettlementApplicant($case_no);
        //var_dump($settlement['settlement_applicant']);die();
        $settlement['settlement_dag_details'] = $this->BhoodanModel->getSettlementDagDetails($case_no);
        $settlement['settlement_dag_area'] = $this->BhoodanModel->getSettlementDagArea($case_no);
        $settlement['settlement_proceeding'] = $this->BhoodanModel->getSettlementProceeding($case_no);
        $settlement['settlement_ap_lmnote'] = $this->BhoodanModel->getSettlementLmNote($case_no);
        $settlement['supportive_document'] = $this->BhoodanModel->getSupportiveDocuments($case_no);
        $settlement['proceedings']   = $this->BhoodanModel->getSettlementProceeding($case_no);
        // Newly added on 08/09/2022
        $settlement['applicants_buyers']   = $this->BhoodanModel->getAllApplicantBuyers($case_no);
        $settlement['applicants_owners']   = $this->BhoodanModel->getAllApplicantOwners($case_no);
        $settlement['applicants_encroacher']   = $this->BhoodanModel->getAllApplicantEncroacher($case_no);
        $settlement['applicants_riotee_nok']   = $this->BhoodanModel->getAllApplicantRioteeNok($case_no);

        // New Added Settlemet Reservation Details
        $settlement['roadside_reservation']   = $this->BhoodanModel->getSettlementRoadsideReservation($case_no);
        $settlement['family_reservation']   = $this->BhoodanModel->getSettlementFamilyReservation($case_no);
        $settlement['vgr_reservation']   = $this->BhoodanModel->getSettlementVgrReservation($case_no);

        // Premium Calculation Details
        if($settlement['settlement_basic']["service_code"] != SETTLEMENT_SPECIAL_CULTIVATORS_ID)
        {
            $this->db2->trans_begin();
            $settlement_premium_insertion = $this->BhoodanModel->premiumReCalculation($this->db2,$case_no);
        ...
        {
            $this->db2->trans_begin();
            $settlement_premium_insertion = $this->BhoodanModel->premiumReCalculationTea($this->db2,$case_no);
        ...
        $settlement['premium_data']  = $this->BhoodanModel->getSettlementPremium($case_no);
        $settlement['landmark_data'] = $this->BhoodanModel->getSettlementDagArea($case_no);
        $settlement['possession_data']   = $this->BhoodanModel->getAllPossessionDetails($case_no);


        //*******getting the deleted settlement_dag_details data from settlement_deleted_data table */
        $deletedEnc=$this->BhoodanModel->getDeletedEncroacher($case_no);
        ...
        $deletedDags=$this->BhoodanModel->getDeletedDags($case_no);
        $deletedData = array();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($dist_code == '' and $caseNo == ''  and $applicationNo == '' and $serviceType == '' and $appStatus == '' and $pendingOffice == '' and $fromDate == '' and $toDate == '' and $cir_code == '') {
                ...
                if ($caseNo != '') {
                    ...
                    $cases = $this->BhoodanModel->getCasesByCaseNo($caseNo);
                    ...
                    $cases = $this->BhoodanModel->getCasesByApplicationNo($applicationNo);
                    $data['cases']      = $cases->result();
                    $data['casesCount'] = $cases->num_rows();
                    ...
                    if ($fromDate != '' and $toDate != '') {
                        ...
                        $cases = $this->BhoodanModel->getCasesByRespectedDataWithDateRage($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
                        ...
                        $cases = $this->BhoodanModel->getCasesByRespectedData($serviceType, $appStatus, $pendingOffice, $fromDate, $toDate, $cir_code);
    ...
    public function searchProposalWIthData()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->BhoodanModel->getProposalsByRespectedData($serviceType, $caseNo);
    ...
    public function vlbEncroacherDetails()
    {
        if (isset($_GET['dag']) && isset($_GET['m']) && isset($_GET['l']) && isset($_GET['v']) && isset($_GET['dist']) && isset($_GET['cir']) && isset($_GET['sub_div'])) {
            ...
            $vlb_encroacher = $this->BhoodanModel->getEncroacherDetails($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_townprt_code, $dag_no);
            $vlb_enc['vlb_enc'] = $vlb_encroacher;
            if ($vlb_encroacher == true) {
                // getting the encroacher details
                $vlb_encroacher_in_dag = $this->BhoodanModel->getEncroacherInDag($vlb_encroacher->id);
    ...
    public function getAllApplicationUnderProposalDept()
    {
        ...
        $pendingCase = $this->BhoodanModel->getAllCasesUnderProposalDept($proposal_no, $dist_code);

        $remainingCase = $this->BhoodanModel->getRemainingCasesUnderProposalDept($proposal_no, $dist_code);
    ...
    public function searchProposalIdByAppCaseNo()
    {
        ...
        if (trim($this->input->post('caseNo')) != null or trim($this->input->post('caseNo')) != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $countApplicationDetails = $this->BhoodanModel->countApplicationDetailsByAppNo($app_no);
                ...
                $getApplicationDetails = $this->BhoodanModel->getApplicationDetailsByAppNo($app_no);
        ...
        $countCaseIdSdlacProposal = $this->BhoodanModel->countProposalIdByCaseNo($case_no);
        ...
        $getSdlacProposalID = $this->BhoodanModel->getProposalIdByCaseNo($case_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($allSelectedList)) {
                ...
                foreach ($allSelectedList as $row) {
                    ...
                    if ($this->BhoodanModel->updateSettlementBasicData($case_no, $updateData) <= 0) {
                        $this->db2->trans_rollback();
                    ...
                    if ($this->BhoodanModel->updateProposalData($case_no, $updateProposalData) <= 0) {
                        $this->db2->trans_rollback();
                    ...
                    //////////////POST To basundhara/////////////////////
                    $application_no = $this->BhoodanModel->getApplicationNoByCaseNo($this->db2,$case_no)->applid;
                    ...
                    $rtps_status = $this->BhoodanModel->postApiBasundhara($rtpsno, $case, $rmk, $status, $task, $pen);
    ...
    public function getAllProposalListUnderSdlac()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->BhoodanModel->getSdlacProposalsByService($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
                $data['proposalsCount'] = $proposals->num_rows();
                $data['_view'] = 'SettlementView/Sdlac/ProposalSearchPageSdlac';
                $this->load->view('layouts/main', $data);
    ...
    public function viewAllApplicationUnderProposalSdlac()
    {
        ...
        $pendingCase = $this->BhoodanModel->getAllCasesUnderProposalSdlac($proposal_no, $dist_code);
    ...
    public function agreeProposalBySdlac()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($proposal_no != NULL && $dist_code != NULL && $service_code != NULL && $sdlac_user_code != NULL) {
                ...
                $updateSdlacMemberReportStatus = $this->BhoodanModel->updateSettlementSdlacMemberReport($proposal_no, $dist_code, $service_code, $sdlac_user_code, $updateData);
                log_message('error', $this->db2->last_query());
                ...
                $updateProposalListQuery = $this->BhoodanModel->updateSettlementProposalList($proposal_no, $dist_code, $service_code, $updateProposalList);
   ...
   public function loginSdlacWithoutAdhaar(){
    ...
    public function getAllSdlacProposalMeeting()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $proposals = $this->BhoodanModel->getSdlacProposalsMeetingByService($dist_code,$serviceType, $sdlac_user_code);
    ...
    public function searchCasesSdlac()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $circles = $this->BhoodanModel->getcirclebyDistCode($dist_code);
    ...
    public function getCirCodeJson($dist_code)
    {
        ...
        $data = $this->BhoodanModel->getCirCodeJSON($dist_code);
        $json = array();
    ...
    public function viewSdlacCaseLIst()
    {
        ...
        $this->db2 =  $this->dbswitch2($dist_code);
        $results = $this->BhoodanModel->viewSdlacSearchData($start, $length, $order, $searchByCol_0,$cir_code,$serviceType,$appStatus,$caseNo,$applicationNo,$pendingOffice,$fromDate,$toDate);
    ...
    public function getAllProposalListUnderSdlacOnlineOffline()
    {
        if ($this->session->userdata('designation') == SDLAC_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $proposals = $this->BhoodanModel->getSdlacProposalsByServiceOnlineOffline($serviceType, $sdlac_user_code);
                $data['proposals']  = $proposals->result();
    ...
    public function getMeetingListByDistrict()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $meetings = $this->BhoodanModel->getMeetingsByDistrict();
    ...
    public function getAllProposalsUnderDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $proposal = $this->BhoodanModel->getProposalsByMeeting($meeting_id);
    ...
    public function getAllCasesUnderProposalDept()
    {
        ...
        $pendingCase = $this->BhoodanModel->getAllCasesUnderProposal($proposal_no);
    ...
    public function downloadCasesWithProposalId()
    {
        ...
        $this->BhoodanModel->downloadExcelReport($file_name, $data);
    }
    ...
    public function checkListForAllotment()
    {
        ...
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE) {
            ...
            $settlement['settlement_basic'] = $this->BhoodanModel->getSettlementBasic($case_no);
            $settlement['settlement_enc'] = $this->BhoodanModel->getSettlementEncroacher($case_no);
            $settlement['proceedings']   = $this->BhoodanModel->getSettlementProceeding($case_no);
            $settlement['roadside_reservation']   = $this->BhoodanModel->getSettlementRoadsideReservation($case_no);
            $settlement['family_reservation']   = $this->BhoodanModel->getSettlementFamilyReservation($case_no);
            $settlement['settlement_applicant']  = $this->BhoodanModel->getSettlementApplicant($case_no);
            ...
            $settlement['settlement_dag_area'] = $this->BhoodanModel->getSettlementDagArea($case_no);
            $settlement['trace_map'] = $this->BhoodanModel->getTraceMap($case_no);
            $settlement['field_report'] = $this->BhoodanModel->getFieldReport($case_no);
            $settlement['geo_tag'] = $this->BhoodanModel->getGeoTag($case_no);
            $settlement['settlement_ap_lmnote'] = $this->BhoodanModel->getSettlementLmNote($case_no);
            $settlement['applicants_buyers']   = $this->BhoodanModel->getAllApplicantBuyers($case_no);
    ...
    public function verifyCaseByAssistant()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $caseCount = $this->BhoodanModel->countSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
            ...
            if ($caseCount == 0) {
                ...
                $this->db2->trans_begin();
                if ($this->BhoodanModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData) == 0) {
                    $this->db2->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function viewAllCabinetCaseList()
    {
        ...
        $results = $this->BhoodanModel->viewCabinetCaseList($start, $length, $order, $dist_code);
    ...
    public function downloadRecommendedCaseReportByDist()
    {
        ...
        if (!empty($allSelectedList)) {
            foreach ($allSelectedList as $row) {
                ...
                $settlement_basic = $this->BhoodanModel->getSettlementBasic($case_no);

                $applicants   = $this->BhoodanModel->getAllApplicantBuyersName($case_no)->result();
                ...
                $dags = $this->BhoodanModel->getAllAppliedDags($case_no)->result();
                ...
                $sdlacStatus = $this->BhoodanModel->sdlacCaseStatus($case_no)->row();

                $settlement_enc = $this->BhoodanModel->getSettlementEncroacher($case_no);
                ...
                $landless = $this->BhoodanModel->getLandLessVerify($case_no)->row();

                $geo_tag = $this->BhoodanModel->getGeoTag($case_no);
        ...
        $this->BhoodanModel->downloadExcelReport($file_name, $finalArray);
    }
    ...
    public function viewAllCasesUnderProposalDept()
    {
        ...
        $results = $this->BhoodanModel->getAllCasesUnderProposal2($start, $length, $order, $dist_code);
    ...
    public function getAllCasesListByDept()
    {
        ...
        if ($this->session->userdata('designation') == DPT_JS) {
            ...
            $data['user_dist']      = $this->BhoodanModel->getDeptUserDistListWithCaseCount();
            $dist_code     = trim($this->input->post('selectDistrict'));
            ...
            if ($this->form_validation->run() == FALSE) 
            {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'Bhoodan/Dept/AllPendingCaseListUnderDpt';
                $this->load->view('layouts/main', $data);
            } 
            else
            {
                ...
                $this->db2 =  $this->dbswitch2($dist_code);
                $meeting = $this->BhoodanModel->getMeetingListByDist($this->db2,$dist_code);
                ...
                log_message('error','--'.$this->db->last_query());
                $cabIdList = $this->BhoodanModel->getCabinetIdList($dist_code,$user_code)->result();
    ...
    public function downloadReportForCabMemo()
    {
        ...
        $dist_codes = $this->BhoodanModel->getDistrictUnderCabMemo($this->db,$cab_memo_id)->result();
        ...
        foreach ($dist_codes as $dist) {
            $allSelectedList = $this->BhoodanModel->getCasesUnderCabMemo($this->db,$cab_memo_id, $dist)->result();
    ...
    public function clearCaseData(){
        ...
        foreach($district as $dist)
        {
            ...
            $allSelectedList = $this->BhoodanModel->getCasesUnderDistCabMemo($district)->result();
            // Switch DB based on dist_code
            $this->db2 = $this->dbswitch2($district);
            ...
            if (!empty($allSelectedList)) {
                foreach ($allSelectedList as $row) {
                    ...
                    $update_basic = $this->BhoodanModel->clearBasicData($where, $updateData);
                }
            }
        }
    ...
    $clearCabDetails = $this->BhoodanModel->clearCabDetails();
  ...
  public function getAllCasesUnderDept() {
    ...
    // die;
    $cases_list = $this->BhoodanModel->getAllCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code,$meeting_no,$verification,$pullRequest);
    // var_dump($cases_list);
    // die;
    ...
    if(!empty($cases_list)) {
      ...
      if($cases_list['total_records'] > 0){
        ...
        foreach($data_rows as $row) {
          ...
          $link = base_url() . "index.php/Bhoodan/settlementBasuNew/?app=".$application_no . "&dist_code=" .$dist_code;
  ...
  function settlementBasuNew()
    {
        ...
        if (isset($case_no)) {
            ...
            } elseif ($output->application->service_code == BHOODAN_LAND_ID) {
                $settlement['service_name'] = 'BHUDAN LAND';
                $settlement['_view'] = 'SettlementView/Dept/BhoodanView.php';
            }
        } else {
            $settlement['_view'] = 'SettlementView/Dept/CaseNotFound';
        }
    ...
    $cases_list = $this->BhoodanModel->getAllCasesUnderAssistant($this->db2,$start, $length, $order,$service_code,$meeting_no);
            ...
            $user_code = $this->session->userdata('user_code');
                ...
                if (!empty($allSelectedList)) {
                    ...
                    foreach ($allSelectedList as $row) {
                        ...
                        $this->db2->trans_begin();
                        $checkCasesSendStatus = $this->BhoodanModel->checkCaseStatusVerify($case_no, $dist_code);
                        ...
                        $updateBasicStatus = $this->BhoodanModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);
    ...
    public function casesListForFinalApprovalByDept()
    {
        ...
        $user_code   = $this->session->userdata('user_code');
        $this->db = $this->load->database('db2', TRUE);
        $cases_list = $this->BhoodanModel->getAllCasesFromMemoForFinalApproval($this->db,$cab_id,$user_code);

        $digitalSignedStatus = $this->BhoodanModel->digitalSignedStatusOfCabId($this->db,$cab_id);
        ...
        $data['_view'] = 'Bhoodan/Dept/caseListForFinalApproval';
        $this->load->view('layouts/main', $data);
    ...
    public function revertToDcByDeptTemp()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $updateBasicStatus = $this->BhoodanModel->updateSettlementBasicData($case_no, $updateBasicData);
            ...
            //////proceeding start//////
                ...
                $insertProceeding = $this->db2->insert('settlement_proceeding', $insPetProceed);
                    ...
                    if ($insertProceeding != 1) {
                        ...
                        $updateProposalRevert = $this->BhoodanModel->updateProposalData($case_no, $updateProposalData);
                                    ...
                                    // var_dump($rtps_status);
                                     ...
                                     if (trim($rtps_status) != 'y') {
                                        ...
                                        $this->db = $this->load->database('db2', TRUE);
                                            ...
                                            $updateCabMemoList = $this->BhoodanModel->updateCabMemoList($updateData, $where);
    ...
    public function finalSubmissionByDept()
    {
            ...
            } else 
            {
                ...
                $this->db = $this->load->database('db2', TRUE);
                $dist_codes = $this->BhoodanModel->getDistrictUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                foreach($distMeeting as $dist)
                {
                    ...
                    $meetingCase = $this->BhoodanModel->getCasesCountByDistMeeting($this->db2,$dist->meeting_id);

                    $pullRequestCase = $this->BhoodanModel->getCasesHavingPullRequest($this->db2,$dist->meeting_id);
                ...
                if($errCheck == 0)
                {
                    ...
                    {
                        $this->db = $this->load->database('db2', TRUE);
                        $caseListForFinalSubmitByDist = $this->BhoodanModel->getCaseListDetailsForFinalSubmit($this->db,$cabinet_id,$user_code,$dist->dist_code)->result();
                    ...
                    $affectedRows = $this->BhoodanModel->updateCabStatus($this->db,$whereUpload, $updateUpload);
                    if($affectedRows <= 0){
    ...
    public function getRevertedCasesDetails()
    {
        ...
        $revertedCases = $this->BhoodanModel->getAllRevertedCaseDetails($cabId);
    ...
    public function getAllRevertedCasesListByDept()
    {
        if ($this->session->userdata('designation') == DEPARTMENT_USERCODE || $this->session->userdata('designation') == DPT_JS) {
            ...
            $data['user_dist']      = $this->BhoodanModel->getDeptUserDistListWithRevertedCaseCount();
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['casesCount'] = 0;
                $this->session->set_flashdata('message', 'Please Select District Before Proceed.');
                $data['_view'] = 'Bhoodan/Dept/AllRevertedCaseListUnderDpt';
                $this->load->view('layouts/main', $data);
            } else {
    ...
    $cases_list = $this->BhoodanModel->getAllRevertedCasesUnderDepartmentAll($this->db2,$start, $length, $order,$service_code);
    ...
    public function downloadRevertedCaseListReport()
    {
        ...
        foreach ($distMeeting as $dist) {
            ...
            $meetingCaseReverted = $this->BhoodanModel->getRevertedCasesCountByDistMeeting($this->db2,$dist->meeting_id)->result();
    ...
    public function downloadRevertedCaseListReportOnlyRemark()
    {
        ...
        $currentCabCreationDate  = $cab_id_date->created_at;
            ...
            $deptCaseReverted = $this->BhoodanModel->getRevertedCasesByDptBeforeCabApproval($this->db2,$cab_finalized_date, $cab_approve_date)->result();

            // $caseListNotUnderCabMemo = $this->BhoodanModel->ListOfRevertCasesNotUnderCabMemo($this->db2,$dist->dist_code,$currentCabCreationDate,$cab_id_date->id);
    ...
    public function getRemarksDetailsRevertedCases()
    {
        ...
        $this->form_validation->set_rules('district_id', 'District ID', 'trim|required|is_natural');
         ...
         if ($this->form_validation->run() == FALSE) {
            ...
            $this->db2 =  $this->dbswitch2($dist_code);
            $revertedCases = $this->BhoodanModel->getDetailsToBeRevertedCase($this->db2,$commaSeparatedCases);
            echo json_encode($revertedCases);
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    }else
                    {
                        ...
                        $task   = MB_DEPARTMENT;
                        $pen    = MB_DEPUTY_COMM;
                        $rtps_status = $this->BhoodanModel->applicationStatusUpdateBulk($concatenatedAppNo,'NA',$rmk,$status,$task,$pen);
                        ...
                        else
                        {
                            ...
                            foreach($casesArray as $rows)
                            {
                                ...
                                $revert_remarks = strtok('(@)');
                                $service_code = strtok('(@)');
                                $revertInsertStatus = $this->BhoodanModel->addIntoRevertList($case_no,$revert_remarks,$service_code,$user_code,$dist_code);
    ...
    public function bulkApproveCasesByDPT()
    {
        ...
        } else
        {
                ...
                $caseWithDist = $this->BhoodanModel->getDistrictCasesUnderCabMemo($this->db,$cabinet_id)->result();
                ...
                if($errCheck == 0)
                {
                    ...
                    $updateCabMemoStatus = $this->BhoodanModel->updateCabMemoList($updateCabMemoData, $whereMemo);
                    ...
                    $updateCabIdStatus = $this->BhoodanModel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                                        ...
                                        }else
                                        {
                                            ...
                                            $updateProposalData = $this->db2->update_batch('settlement_proposal_cases',$updateProposalData,'case_no');
                                                ...
                                                else
                                                {
                                                    ...
                                                    $cabIdUpdate = $this->BhoodanModel->updateCabStatus($this->db,$whereCabID, $updateCabPartial);
                                                    if($cabIdUpdate <= 0){
        ...
        } else 
        {
            ...
            $errCheck = 0;
                ...
                if(CHITHA_AREA_VALIDATION == 1)
                {
                    ...
                    {
                        $checkArea = $this->BhoodanModel->chithaReserveAreaCheckWithCaseNo($this->db2,$case); 
                ...
                $pullRequestCase = $this->BhoodanModel->getPullRequestStatus($this->db2,$casesList);
                $pullRequestCaseCount = $pullRequestCase->num_rows();
                ...
                if($errCheck == 0)
                {
                    ...
                    $user_code = $this->session->userdata('user_code');
                    $this->db = $this->load->database('db2', TRUE);
                    $memo_name = $this->BhoodanModel->getMemoNameByCabId($this->db,$cabmemo_id);
                        ...
                        if (!empty($allSelectedList)) 
                            {
                                ...
                                $cabMemoCases = $this->BhoodanModel->getExistingCasesUnderCabMemo($dist_code);
                                ...
                                if (!empty($duplicates)) 
                                {
                                    ...
                                    $this->db2 =  $this->dbswitch2($dist_code);
                                    $checkPullRequestFinalStatus = $this->BhoodanModel->checkPullRequestFinalStatus($this->db2,$duplicateCases)->result();
                                        ...
                                        foreach($casesExist as $casesE)
                                            {
                                                ...
                                                $this->db = $this->load->database('db2', TRUE);
                                                $getOldCaseDetails = $this->BhoodanModel->getOldCaseDetailsFromCabMemo($this->db,$caseNoOld,$dist_code)->row();

                                                $oldCaseNo = $getOldCaseDetails->case_no;

                                                $this->db2 =  $this->dbswitch2($dist_code);
                                                $getMeetingProposal = $this->BhoodanModel->getMeetingProposalByCaseNo($this->db2,$caseNoOld)->row();
                                                    ...
                                                    $caseBackupDetails = [
                                                        ...
                                                        {
                                                            $updateCabMemoList = $this->BhoodanModel->updateCabMemoList($newCaseDetails,$whereCase);
                                                            ...
                                                            if ($updateCabMemoList <= 0) {
                                                                ...
                                                                $updateCabIdListStatus = $this->BhoodanModel->updateCabStatus($this->db,$whereCabId, $updateCabIdData);
                                                                ...
                                                                return false;
                                                                    ...
                                                                    $updateBasicStatusOld = $this->BhoodanModel->updateSettlementBasicForCab($this->db2,$oldCaseNo, $dist_code, $updateBasicData);
                                ...
                                if (!empty($newCases)) 
                                {
                                    foreach ($newCases as $caseN) 
                                    {
                                        ...
                                        $this->db2 =  $this->dbswitch2($dist_code);
                                        $getMeetingProposal = $this->BhoodanModel->getMeetingProposalByCaseNo($this->db2,$case_no)->row();
                                        ...
                                        if ($insertCabList != TRUE) {
                                            ...
                                            $updateCabIdList = $this->BhoodanModel->updateCabStatus($this->db,$where, $updateData);
                                            if ($updateCabIdList <= 0) {
                                                ...
                                                $updateBasicStatusNew = $this->BhoodanModel->updateSettlementBasicForCab($this->db2,$case_no, $dist_code, $updateData);
