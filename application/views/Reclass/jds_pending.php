    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            <div class="reza-title">
                <span>Pending Reclassuite Cases (Forwarded by ADC)<span style="color:red;font-size: 14px;"> ( N.B: Please Select District Before Proceed )</span></span>
            </div>
            ...
            <div class="reza-body">
                    ...
                    <div class="row" id="searchBox">
                        ...
                        <div class="col-lg-6">
                            <button type="submit" id="dist_select_btn_submit" class="rezaButt buttInfo" style="width: 200px">
...
<div class="modal" role="dialog" id="markVerificationModal" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
		...
		<div class="form-group">
                    <label>JDS Remark for above cases : </label>
                    <textarea class="form-control" name="jds_remark" id="jds_remark"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" id="modalClose">NO</button>
                <button type="button" class="btn btn-primary btn-sm" id="confirmSentForVerificationSO">Yes Send to ADS</button>
            </div>
        </div>
    </div>
</div>
<!-- Modal for sent case for Verification End -->
<!-- Modal for sent case for Verification DLR -->
<div class="modal" role="dialog" id="markVerificationModalDLR" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success">
                <h5 class="modal-title text-center" id="exampleModalLongTitle"></h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>JDS Remark for above cases : </label>
                    <textarea class="form-control" name="jds_remark_dlr" id="jds_remark_dlr"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" id="modalClose">NO</button>
                <button type="button" class="btn btn-primary btn-sm" id="confirmSentForVerificationToDLR">Yes forward to DLR</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $('#dist_select_btn_submit').on('click', () => {
            ...
            $.ajax({
                url: baseurl + "ReclassificationSuiteSurvey/viewReclassCases" ,
                type: "POST",
	...
	$('#revertToDCModal').modal('hide');
	        $('#markVerificationModalDLR').modal('hide');
    });
...
$(document).ready(function() {
    ...
    $(document).on('click', '#sentForVerification', function() {
        ...
        $('#markVerificationModal .modal-title').text('Cases for ADS (Total Selected Cases: ' + selectedList.length + ')');
        $('#markVerificationModal').modal('show');
    } else {
        showWarningMessage("Please Select Case Before Send to ADS");
    }
});

    $(document).on('click', '#sentForVerificationtoDLR', function() {
        var district_id = $("#selectDistrict").val();
        selectedList = [];
        $('.selectMark:checked').each(function(i) {
            selectedList[i] = $(this).val();
        ...
        $('#markVerificationModalDLR .modal-title').text('Cases for DLR (Total Selected Cases: ' + selectedList.length + ')');
        $('#markVerificationModalDLR').modal('show');
    } else {
        showWarningMessage("Please Select Case Before Send to DLR");
    }
    });


    $(document).on('click', '#confirmSentForVerificationToDLR', function() {
        var district_id = $("#selectDistrict").val();
        var jds_remark = $("#jds_remark_dlr").val();
        ...
        if (selectedList.length > 0) {
            ...
            $.ajax({
                ...
                url: baseurl + "ReclassificationSuiteSurvey/sentReclassCasestoDLR",
                type: "POST",
                dataType: "json",
    ...
    $(document).on('click', '#confirmSentForVerificationSO', function() {
        ...
        if (selectedList.length > 0) {
            ...
            $.ajax({
                // url: baseurl + "DeptConversion/sentConversionCasesForVerification",
                url: baseurl + "ReclassificationSuiteSurvey/sentReclassCasesADS",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                success: function(data) {
    ...
    $(document).on('click', '#confirmAddCasesToProposal', function() {
        ...
        if (selectedList.length > 0) {
            ...
            $.ajax({
                url: baseurl + "DeptConversion/addConversionCasesToProposal",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                success: function(data) {
    ...
    if (selectedList.length > 0) {
        ...
        $.ajax({
            url: baseurl + "DeptConversion/getAstRemarksDetailsRevertedCases",
            type: "POST",
            dataType: "json",
            contentType: "application/json",
            data: JSON.stringify(applicant),
