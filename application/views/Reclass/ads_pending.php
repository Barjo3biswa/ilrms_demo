    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            <div class="reza-title">
                <span>Pending Reclassuite Cases (Forwarded by JDS)<span style="color:red;font-size: 14px;"> ( N.B: Please Select District Before Proceed )</span></span>
            </div>
		...
		<div class="row">
                    <div class="col-lg-2 col-md-2 col-sm-4 col-sx-12">
                      ...
                      <div class="col-lg-4 col-md-4 col-sm-4 col-sx-12">
                        <div class="form-group">
                          <input type="text" class="form-control" id="file_name" name="file_name" style="width: 100%" placeholder="Enter document Name">
                        </div>
                      </div>
                      <div class="col-lg-6 col-md-6 col-sm-6 col-sx-12">
                        <div class="form-group">
                          <input type="file" id="up_file" name="up_file" class="file__input" accept="image/png, image/jpeg, application/pdf">
                        </div>
                      </div>
            
                </div>
		<div class="form-group">
                    <label>ADS Remark for above cases : </label>
                    <textarea class="form-control" name="ads_remark" id="ads_remark"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" id="modalClose">NO</button>
                <button type="button" class="btn btn-primary btn-sm" id="confirmSentForVerificationSO">Yes verified and send to JDS</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $('#dist_select_btn_submit').on('click', () => {
            ...
            $.ajax({
                url: baseurl + "ReclassificationSuiteSurvey/viewReclassCasesAds" ,
                type: "POST",
...
$(document).ready(function() {
    ...
    $(document).on('click', '#confirmSentForVerificationSO', function() {
        ...
        var district_id = $("#selectDistrict").val();
        var ads_remark = $("#ads_remark").val();

        if (selectedList.length > 0) {
            var uploadedFile = new FormData($("#uploadData")[0]);
            uploadedFile.append("up_file", up_file.files[0]);    
            uploadedFile.append("file_name", $("#file_name").val());
            uploadedFile.append("district_id", $("#selectDistrict").val());
            uploadedFile.append("ads_remark", $("#ads_remark").val());
            uploadedFile.append("selectedList[]", selectedList);
            $.ajax({
                // url: baseurl + "DeptConversion/sentConversionCasesForVerification",
                url: baseurl + "ReclassificationSuiteSurvey/sentReclassCasetoJDSbyADS",
                type: "POST",
                dataType: "json",
                    ...
                    if (data.responseType == 1) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $('#dataTableReclassJDS').DataTable().ajax.reload(null, false);
                                $('#markVerificationModal').modal('hide');
