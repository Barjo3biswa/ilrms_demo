...
<div class="modal" role="dialog" id="markVerificationModal" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-lg" role="document">
		...
		<div class="form-group">
                    <label>DLR Remark for above cases : </label>
                    <textarea class="form-control" name="dlr_remark" id="dlr_remark"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary btn-sm" id="modalClose">NO</button>
                <button type="button" class="btn btn-primary btn-sm" id="confirmSentForVerificationDlr">Forward</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $('#dist_select_btn_submit').on('click', () => {
            ...
            $.ajax({
                url: baseurl + "ReclassificationSuiteSurvey/viewReclassCasesDlr" ,
                type: "POST",
...
$(document).ready(function() {
    ...
    $(document).on('click', '#sentForVerificationDlr', function() {
        ...
        $('#markVerificationModal .modal-title').text('Cases for JDS (Total Selected Cases: ' + selectedList.length + ')');
        $('#markVerificationModal').modal('show');
    } else {
        showWarningMessage("Please Select Case Before Send to JDS");
    }
});


    // Handle confirm Sent for verification to SO
    $(document).on('click', '#confirmSentForVerificationDlr', function() {
        var district_id = $("#selectDistrict").val();
        var dlr_remark  = $("#dlr_remark").val();
        if (selectedList.length > 0) {
            const applicant = {
                selectedList: selectedList,
			district_id: district_id,
			dlr_remark:dlr_remark
            };
            $.ajax({
                url: baseurl + "ReclassificationSuiteSurvey/sentReclassCasesJDSbyDLR",
                type: "POST",
                dataType: "json",
            ...
            showWarningMessage("Please Select Case Before Approval");
        }
    });
