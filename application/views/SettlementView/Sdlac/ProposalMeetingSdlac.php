    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            ...
            <div class="reza-body">
                ...
                <form action="<?php echo base_url(); ?>index.php/Basundhara/getAllSdlacProposalMeeting" method="post">
                    ...
                    <table class='table table-striped table-bordered' id='dataTable' width="100%">
                        ...
                        <tbody>
                            ...
                            foreach ($proposals as $proposal) :  $i++ ?>
                                <tr>
                                    <td><?php echo $i ?> </td>
                                    <td><strong class="text-primary">Proposal No <?php echo $proposal->id; ?></strong>
                                    </td>
                                    <td class="center">
                                    ...
                                    <td class="text-center">
                                        ...
                                        <!-- <form target="_blank" action="<?php echo base_url();?>index.php/Basundhara/viewInvitationNotice" method="post">
                                            <input type="hidden"  name="dist_id_invite" value="<?php echo $proposal->dist_code; ?>">
                                            <input type="hidden"  name="proposal_id_invite" value="<?php echo $proposal->id; ?>">
                                            <button type="submit" class="btn btn-primary btn-sm mb-2">View Meeting Notice <i class="fa fa-bell"></i></i></button>
                                        </form> -->
                                        
                                        <a class="btn btn-secondary btn-sm" target="_blank" href="<?php echo base_url(); ?>index.php/Basundhara/viewAllApplicationUnderProposalSdlac/?proposal=<?php echo $proposal->id; ?>&service_code=<?php echo $service_code; ?>">
                                            <i class="fa fa-eye"></i></i> View All Cases
                                        </a>

                                        <button class="btn btn-danger btn-sm"  id="openSdlacMeeting" value="<?php echo $proposal->id; ?>">
                                            <i class="fa fa-bell"></i> Meeting Notice
                                        </button>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header text-white text-bold text-center">
                <h4 class="modal-title text-danger text-center"><i class="fa fa-bell"></i>  Invitation Notice Proposal No :<span id="meeting_proposal_id"></span></h4>
                <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">X</button>
            </div>
            <div class="modal-body" id="meetingNoticeView">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">X</button>
            </div>
...
<script>
        $(document).on('click', '#openSdlacMeeting', function() {

        $('#meeting_proposal_id').text($(this).val());
        var proposal_id_invite = $('#meeting_proposal_id').val();

         const applicant = {
                proposal_id: proposal_id_invite,
            };
            console.log(applicant);
            $.ajax({
            url: '<?php echo base_url() . "index.php/Basundhara/viewInvitationNotice" ?>',
            type: "post",
            dataType: "json",
            ...
            success: function(data) {
                if (data.responseType == 1) {
                    ...
                    $('#viewProposalMinutesSdlacModal').modal({
                    backdrop: 'static',
                    keyboard: false
                    });
                    ...
                    $('#meetingNoticeView').html(noticeDiv);
