    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            ...
            <div class="reza-body">
                ...
                <form action="<?php echo base_url(); ?>index.php/Basundhara/getAllCasesListByDept" method="post">
    ...
    <form method="post" id="case_revert_to_dc_form">
        <div class="modal-dialog modal-dialog-scrollable modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-header text-bold text-center"><strong>Please Select Cab Memo for Revert</strong></div>
                <div class="modal-header text-white text-bold text-center">
                        <select class="form-select" aria-label="Default select example" name="cabMemoIdRevert" id="cabMemoIdRevert" required>
                            <!-- <option disabled selected>-------------Select Cab Memo for Revert----------------</option> -->
                                <?php foreach ($cabIdList as $cab) :  ?>
                                    <option value='<?php echo $cab->cab_id; ?>'><?php echo substr($cab->cab_memo_name,0,90); ?> &nbsp;(<?php echo $cab->cab_id; ?> ) &nbsp;(<?php echo date('d-M-Y', strtotime($cab->created_at)); ?>)</option>
                                <?php endforeach; ?>
                        </select>
                </div>
...
<script>
    ...
    $(document).on('click', '#selectCabMemoForRevert', function() {
        ...
        if (selectedList.length > 0) {
            ...
            $('#selectCabMemoRevertModal').modal('show');
    ...
    $(document).on('click', '#markAndRecommendedWithoutVerify', function() {
        ...
        $('.selectMark:checked').each(function(i) {
            ...
            $.ajax({
                url: BASE_URL + "/Basundhara/addCasesToCabMemo",
                type: "POST",
                dataType: "json",
    ...
    $(document).ready(function() {
        ...
        function load_data_cab_case_list(select_service,select_meeting,select_verify,select_pull_request) {
            ...
            var table = $('#dataTableDeptByDist').DataTable({
                ...
                'ajax': {
                    url: BASE_URL+'index.php/Basundhara/getAllCasesUnderDept',
                    type: 'POST',
                    data: {
    ...
    event.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $('#show-Img').show();
                $.ajax({
                    url: BASE_URL + "/Basundhara/bulkRevertCasesToDC",
                    type: "POST",
                    data: formdata,
                    dataType: "json",
                    success: function(data) {
