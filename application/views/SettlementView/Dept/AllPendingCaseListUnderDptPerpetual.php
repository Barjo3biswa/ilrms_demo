    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            ...
            <div class="reza-body">
                ...
                <form action="<?php echo base_url(); ?>index.php/BasundharaMb2Perpetual/getAllCasesListByDept" method="post">
                    ...
                    <select class="form-select" aria-label="Default select example" name="selectCabMemo" id="cabinetMemo" required>
                        <option disabled selected>-------------Select Cab Memo----------------</option>
                            <?php foreach ($cabIdList as $cab) :  ?>
                                <option value='<?php echo $cab->cab_id; ?>'><?php echo $cab->cab_memo_name; ?>&nbsp;&nbsp;&nbsp;( Cab ID: <?php echo $cab->cab_id; ?> )&nbsp;( Cab Type: <?php echo $cab->cab_type; ?> )</option>
                            <?php endforeach; ?>
                    </select>
                </div>
                </div>
                        ...
                        <table class="table table-striped table-bordered " id="reverted_case_details_table"  >
                            <thead>
                                <tr>
                                    ...
                                    <td width="60%">ASO Remarks</td>
                        
                                </tr>
                            </thead>
                            <tbody id="TextBoxContainerViewForm"></tbody>
                        </table>
                        <label>REVERTED REMARKS FROM JOINT SECRETARY</label>
                        <textarea id="reverted_from_js_to_dc" name="reverted_from_js_to_dc" class="form-control" placeholder="Enter Revert reason"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
...
<script>
    ...
    $(document).on('click', '#markAndRecommendedWithoutVerify', function() {
        ...
        $('.selectMark:checked').each(function(i) {
            ...
            $.ajax({
                url: BASE_URL + "/BasundharaMb2Perpetual/addCasesToCabMemo",
                type: "POST",
                dataType: "json",
...
// Datatable
    ...
    $(document).ready(function() {
        ...
        function load_data_cab_case_list(select_service,select_meeting,select_verify,select_pull_request) {
            ...
            var table = $('#dataTableDeptByDist').DataTable({
                ...
                'ajax': {
                    url: BASE_URL+'index.php/BasundharaMb2Perpetual/getAllCasesUnderDept',
                    type: 'POST',
                    data: {
    ...
    $(document).on('click', '#bulkSentForVerification', function() {
        ...
        var selectAssistant = $('#selectAssistant').val();
            ...
            // return;
            $.ajax({
                url: BASE_URL + "/BasundharaMb2Perpetual/bulkSentForVerification",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                success: function(data) {
    ...
    $(document).on('click', '#bulkRevertToDcModalOpen', function() {
        ...
        if (selectedList.length > 0) {
            ...
            console.log(applicant);
                ...
                $.ajax({
                url: BASE_URL + "/BasundharaMb2Perpetual/getRemarksDetailsRevertedCases",
                type: "POST",
                dataType: "json",
                contentType: "application/json",
                success: function (data) {
                    ...
                    for (let i = 0; i < data.reverted_case_list.length; i++) {
                        ...
                        $('#view_remark_'+i).val(data.reverted_case_list[i].verified_ast_remarks).attr('readonly', true);;       
                    }
    ...
    event.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $('#show-Img').show();
                $.ajax({
                    url: BASE_URL + "/BasundharaMb2Perpetual/bulkRevertCasesToDC",
                    type: "POST",
                    data: formdata,
                    dataType: "json",
                    success: function(data) {
