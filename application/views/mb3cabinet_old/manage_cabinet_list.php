...
<?php $services = json_decode(DEPT_MB3_SERVICES)?>
  ...
  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mb-2">
    <div class="reza-card">
      <div class="reza-title">
        <div class="row">
          <div class="col-lg-12">
              <span>Manage Cabinet For MB 3.0 Services</span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="row">
          ...
          <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12 mb-2">
            <div class="form-group">
              <label style="color: #666">Select Service</label>
                <select class="form-select" aria-label="Default select example" id="service_select" name="service_select">
                    <option value="00" selected>SELECT CATEGORY</option>
                    <?php foreach ($services as $key => $ser) { ?>
                        <option value="<?php echo $ser->CODE; ?>"><?php echo $ser->NAME; ?></option>
                    <?php  } ?>
                </select>
            </div>
          </div>
          ...
          <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12 mb-2 ins_master_cat" style="display:none">
            <div class="form-group">
              <label style="color: #666">Select Juridical Category</label>
              <select class="form-select" aria-label="Default select example" id="juridical_cat" name="juridical_cat">
              </select>
            </div>
          </div>
          ...
          <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12 mb-2">
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="cab_memo_name" value="" autocomplete="off" placeholder="Enter CAB Memo Name">
            </div>
          </div>
          <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="form-group">
              <label style="color: #666">Remarks</label>
              <textarea  class="form-control" id="cab_remarks" rows="3" autocomplete="off" placeholder="Enter Remarks"></textarea>
            </div>
          </div>
    ...
    <div class="reza-card">
      ...
      <div class="reza-body" id="showBody">
        <table class="datatable table table-stripped" id='dataTableMb3CabList'>
          <thead>
            <tr>
              <th class="center"><label class="control-label">Cab Memo</th>
              <th class="center"><label class="control-label">Service Name</th>
              ...
              <!-- <th class="center"><label class="control-label">Modify</label></th> -->
...
<script>
  ...
  $(document).on('click', '#createCabId', function(){
    ...
    var ins_cat = '';
    if($('#service_select').val() == 45)
    {
      ins_cat = $('#juridical_cat').val();
    }
    ...
    if (selectedDistricts.length > 0)
    {
      ...
      $.ajax({
        url: baseurl + "DeptMb3Cabinet/generateMb3Cabinet",
        type: "post",
        dataType: "json",
        contentType: "application/json",
        success: function (data) {
  ...
  $(document).ready( function () {
    ...
    {
      var table = $('#dataTableMb3CabList').DataTable({
        'pageLength': 10,
        "processing": true,
        "serverSide": true,
        "ordering"  : false,
        ...
        'ajax':{
          url: baseurl + "DeptMb3Cabinet/getAllMb3CabinetList",
          type:'POST',
          data: {status: 0},
          deferLoading: 57,
        },
  ...
  function viewCaseDetail(cab_id)
  {
    ...
    $.ajax({
      url: baseurl + "DeptMb3Cabinet/createCabId",
      type: "post",
      dataType: "json",
      contentType: "application/json",
      success: function (data) {
  ...
  //Delete VGR Cab 
  function deleteCabMemoMb3(cab_id_delete)
  {
    const cablist = {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
            url: baseurl + "DeptMb3Cabinet/deleteMb3Cabinet",
            type: "post",
            dataType: "json",
            contentType: "application/json",
            success: function (data) {
