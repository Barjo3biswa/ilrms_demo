        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb p-3 text-white">
                <li class="breadcrumb-item font-weight-bold">
                    <a href="<?= base_url().'number-of-dags-lot-wise?dist_code='.$villDagCount[0]->dist_code.'&subdiv='.$villDagCount[0]->subdiv_code.'&cir_code='.$villDagCount[0]->cir_code.'&mouza='.$villDagCount[0]->mouza_pargona_code.'&lot_no='.$villDagCount[0]->lot_no?>">
...
<div class="p-2 bg-primary text-center text-white h5 shadow-sm border border-white rounded"><b>Dag Details Village Wise ( <?= $this->utilityclass->dist_name($villDagCount[0]->dist_code) ?> জিলা )</b>
</div>
            ...
            <table class="table table-hover text-center tab1">
                <thead>
                    <tr style="background-color: #136a6f; color: #fff;">
						...
						<td width="15%">Rural/Urban/Unmapped Dag(s)</td>
                    </tr>
                </thead>
                <tbody>
				<?php $i =1;
				foreach($villDagCount as $key=>$row) : ?>
                    <tr>
						<td><?= ++$key ?></td>
						...
						<td><?= $row->dag ?></td>
						<td>RU: <?= $row->rural ?> / UR: <?= $row->urban ?> / UN: <?= $row->unmapped ?></td>
                    </tr>
				<?php endforeach; ?>
                </tbody>
            </table>
