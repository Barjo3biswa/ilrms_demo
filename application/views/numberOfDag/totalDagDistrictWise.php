...
<div class="p-2 bg-primary text-center text-white h5 shadow-sm border border-white rounded"><b>Dag Details District Wise</b>
</div>
        ...
        <div class="card-body">
            ...
            <table class="table table-hover text-center tab1" id="tab">
                ...
                <tbody>
                    ...
                    <?php
//                        $districtwise = $districtDagCount->result();
//                        $district = json_decode($districtwise[0]->districtwise_json);
						$i= 1;
                        foreach($districtDagCount as $key=>$row) :
                    ?>
                        <tr>
							<td><?= $i++;?></td>
                            <td><?= $this->utilityclass->dist_name($row->district_code) ?></td>
                            <td><?= $row->district_wise_dag ?></td>
							<td><?= $row->district_rural_dag ?></td>
							<td><?= $row->district_urban_dag ?></td>
							<td><?= $row->district_unmapped_dag ?></td>
                            <td align="center">
                                <a href="<?= base_url().'number-of-dags-circle-wise/'.$row->district_code?>">
