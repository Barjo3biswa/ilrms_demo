            ...
            <table class="table table-hover text-center tab1" id="tab">
                <thead>
                    <tr style="background-color: #136a6f; color: #fff;">
                        ...
                        <td width="15%">No of Pattadar(s)</td>
                ...
                <tbody>
                    ...
                    <?php
//                        $districtwise = $districtPattadarCount->result();
//                        $district = json_decode($districtwise[0]->districtwise_json);
						$i=1;
                        foreach($districtPattadarCount as $key=>$row) :
                    ?>
                        <tr>
							<td><?= $i++;?></td>
                            <td><?= $this->utilityclass->dist_name($row->district_code) ?></td>
                            <td><?= $row->district_wise_pattadar ?></td>
                            <td align="center">
                                <a href="<?= base_url().'total-pattadar-circle-wise/'.$row->district_code?>">
