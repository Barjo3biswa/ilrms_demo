    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
      <div class="reza-card">
        <div class="reza-title">
          <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
              <span class="text-danger">Final Approval of VGR CAB Memo: </span></br>
                <p><mark>Do not  Final Approve without Approval from Cabinet</mark></p>
            </div>
...
<div id="generateNotificationViewVGR"></div>

<!-- Modal for Generate Notification Copy  VGR-->
<div class="modal" role="dialog" id="notificationModalVGR" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <hr>
      <form id="generateNotificationPostVGR">
      <div class="modal-body">
                <div class="row"> 
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-secondary" id="closenotificationModalVGR">Close</button>
          </div>
          </form>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  $(document).ready( function () {
    ...
    function load_data(selectDistrict)
    {
      var table = $('#dataTableCab').DataTable({
        ...
        'ajax':{
          url: base_url+'index.php/CabController/getCabIdByUserDistrictFinalApprovalVGR',
          type:'POST',
  ...
  function openModalNotificationVGR(cab_id){
    $('#display_cab_id_memo').html(cab_id);
    $('#cab_id_memo').val(cab_id);
    $('#notificationModalVGR').show('modal');
  }


  $('#generateNotificationPostVGR').on('submit', function(event)
    {
      ...
      var formData = $(this).serialize();
        ...
        $.ajax({
            type        : 'POST', 
            url         : base_url +'CabController/GenerateNotificationVGR', 
            data        : formData, 
            // dataType    : 'json', 
            encode      : true,
            success: function(data){
              console.log(data);
              $('#notificationModalVGR').hide('modal');
              $("#generateNotificationViewVGR").html(data);
              $('#generateNotificationModalVGR').show('modal');
            },
        });
    });

        $('#closeNotificationModalVGR').on('click', function(){
        $('#notificationModalVGR').hide('modal');
      });
