    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="reza-card">
            <div class="reza-title">
                <div class="row">
                      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                          <span class="text-danger">List to be Finalized
                          </span>
                      </div>
                </div>
...
<div class="modal" role="dialog" id="memoModal" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header  bg-success">
        <h5 class="modal-title text-center" id="exampleModalLongTitle">
          Generate Memorendum for Cab No : <span id='display_cab_id_memo'></span>
        </h5>
      </div>
      <hr>
...
<script type="text/javascript">
  ...
  $(document).ready( function () {
    ...
    function load_data(selectDistrict)
    {
      var table = $('#dataTableCab').DataTable({
        ...
        'ajax':{
          url: base_url+'index.php/CabControllerReviewPerpetual/getCabIdByUserDistrict',
          type:'POST',
  ...
  function openModalMemo(cab_id){
    ...
    $.ajax({
      url: base_url + "CabControllerReviewPerpetual/getCasesByCabId",
      type: "post",
      dataType: "json",
      contentType: "application/json",
      success: function (data) {
  ...
  function openModalMemo(cab_id){
    ...
    $('#memoModal').show('modal');

    // $.ajax({
    //   url: base_url + "CabController/generateMemo",
    //   type: "post",
    //   dataType: "json",
    //   contentType: "application/json",
    //   success: function (data) {
...
$(document).ready(function(){
  $('#generateMemoPost').on('submit', function(event){
    ...
    // if($("#idc").val().trim().length < 1)
    // {
    //   alert("Please Enter Your Inter Departmental Consultation");
    //   return; 
    // }
    ...
    var formData = $(this).serialize();
        $.ajax({
            type        : 'POST', 
            url         : base_url +'CabControllerReviewPerpetual/GenerateCabMemo', 
            data        : formData, 
            // dataType    : 'json', 
            encode      : true,
            success: function(data){
              console.log(data);
              $('#memoModal').hide('modal');
              $("#generateMemoView").html(data);
              $('#generatememoModal').show('modal');
            ...
            //       $('#generatememoModal').show('modal');
            //   } else {
            //       showErrorMessage("Memo Generate Failed! Kindly Contack System Admin.");
            //   }
            // },
        });
    });
  ...
  // final submit & generate PDF
    $(document).on("click",'#generatePDFMemo',function(){
        ...
        else
        {
            ...
            $.ajax({
                url: base_url + "CabControllerReviewPerpetual/SavePDFMemo",
                type: "post",
                dataType: "json",
                data: {
