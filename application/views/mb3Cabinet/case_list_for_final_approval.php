    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="reza-card">
            <div class="reza-title">
                <span class="text-danger">List of Cases For Final Approval Under Cab Memo: <?php echo $cab_id ?></span>
                <hr>
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            ...
            <div class="reza-body">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <small class='text-danger'><?php if ($case->case_no) {
                                        ...
                                        $service = $this->DeptMb3CabinetModel->getserviceFromCaseNo($case->case_no);

                                        // var_dump($service); die;
                                        if($service == 'TGPP'){
                                            $applid = $this->utilclass->getApplidFromCaseNo($case->dist_code,$case->case_no);
                                        }elseif($service == 'CONV'){
                                            $applid = $this->utilclass->getApplidFromCaseNoBasundharApplications($case->dist_code,$case->case_no);
                                        }elseif($service == 'SOTU'){
                                            $applid = $this->utilclass->getApplidFromCaseNo($case->dist_code,$case->case_no);
                                        }
                                        elseif($service == 'SLIJ'){
                                            ...
                                            echo "Basundhara:" . $applid;
                                        } ?> 
                                    </small>
                                </td>
                              <!-- <td> meeting id needed</td> -->
                               <?php $getMeetingId = $this->DeptMb3CabinetModel->getMeeting_id_from_case_no($case->case_no);?>
                               <?php if($getMeetingId == null):?>
                                ...
                                <td>
                                    <small>
                                        <input type="hidden" value="<?php echo $case->case_no?>" id="case_no_<?=$case->id?>">
                                                ...
                                                <!-- <br>
                                            <input type="radio" name="report_status_<?=$case->id?>" id="report_status_no<?=$case->id?>" value="2" onclick="report_no('<?=$case->dist_code?>','<?=$case->case_no?>')"   <?php if($case->final_status == 2){{ echo "checked";}}?>>&nbsp;Revert to DC
                ...
                <?php if($digitalSignedStatus ==0){ ?>
                    <div class="col-lg-6">
                            ...
                            <button  class="btn btn-md btn-danger" id="digitalSignNotificationViewId" onclick="digitalSignNotificationView('<?php echo $cab_id ?>')">
...
<div id="generateSignNotificationView"></div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">
                    Final Submission of Cases Under Cab Memo <span id="cabinet_id_submit" class="text-danger"></span>
                    <input type="hidden" value="" id="caninet_id_selected" name="caninet_id_selected">
                </h5>
            </div>
            <div class="modal-body" align="center">

                <h4 class="text-success">Total No of Cases For Final Approval: <span id="TotalNoCases" class="text-danger"></span></h4>
                ...
                <br>
                <!-- <div class="row mt-3">
                    <label class="col-md-5"><b>Order No :</b> </label>
                    <div class="col-md-6">
                    <input type="text" class="form-control"  id="dept_order_no" name="dept_order_no">
                </div>
                </div>
                <div class="row mt-3">
                    <label class="col-md-5"><b>Notification Copy :</b> </label>
                    <div class="col-md-6">
                    <input type="file" name="notificationCopy" id="notificationCopy" required>
                    </div>
                </div> -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-success"  id="finalSubmitByDeptYes">Final Approve & Send for Payment Generation</button>
...
function openFinalSubmitModal(cab_id){
    ...
    $.ajax({
        url: BASE_URL + "DeptMb3Cabinet/getTotalNoCasesForFinalApproval",
        type: 'POST',
        data: formdata,
        dataType: 'json',
        success: function (data) {
            // console.log(data);
            // var caseCount = response.case_count;
            // var notificationNo = response.notification_no;

            $('#cabinet_id_submit').text(cab_id);
            $('#TotalNoCases').text(data);
            // $('#DeptOrderNo').text(notificationNo);
            // $('#dept_order_no').val(notificationNo);
...
function finalSubmit(cab_id) {
    ...
    $(document).on('click', '#finalSubmitByDeptYes', function() {
        ...
        // uploadedFile.append("dept_order_no", $("#dept_order_no").val());
        // uploadedFile.append("fileToUpload", fileToUpload.files[0]);
        // uploadedFile.append("notificationCopy", notificationCopy.files[0]);
        $('#finalSubmissionModal').modal('hide'); 
        Swal.fire({
            title: 'Are you sure?',
            text: "All Cases Will be Sent for Final Submission",
            icon: 'info',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                    ...
                    $('#finalSubmit').hide();
                $.ajax({
                    url: BASE_URL + "/DeptMb3Cabinet/bulkApproveCasesByDPT",
                    type: "POST",
                    dataType: "json",
 ...
 function digitalSignNotificationView(cab_id){
        // alert('hi');
                $.ajax({
                    type        : 'POST', 
                    url         : BASE_URL +'DeptMb3Cabinet/GenerateNotificationForSign', 
                    data        : {cab_id:cab_id}, 
                    // dataType    : 'json', 
                    encode      : true,
                    success: function(data){
                    ...
                    $("#generateSignNotificationView").html(data);
                    $('#generateNotificationModal').show('modal');
                    },
                });
            }

//Get PDF Data in base64
