...
<div class="row rezaBody" style='padding: 10px 20px 20px 0px;'>
    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-12" style="padding-left: 30px; margin-top: 15px">
        ...
        <?php if($ticket->ticket_status == TICKET_STATUS_PENDING && $ticket->a_status == 1) : ?>
            <a href="<?= base_url()?>assign-ticket-list-dev">
                Assigned
            </a>
        <?php elseif($ticket->ticket_status == TICKET_STATUS_PENDING && $ticket->a_status == 2) : ?>
            <a href="<?= base_url()?>request-for-closed-dev">
                Request for Closed
            </a>
        <?php elseif($ticket->ticket_status == TICKET_STATUS_CLOSED) : ?>
            <a href="<?= base_url()?>closed-ticket-list-dev">
                Closed
            </a>
        <?php endif; ?>
        / Details
        ...
        <div class="reza-card">
            <div class="row">
                ...
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12" align="right" style="padding-top: 15px; padding-right: 25px">
                    <?php if($ticket->ticket_status == TICKET_STATUS_PENDING && $ticket->a_status == 1): ?>
                        <button class="rezaButt buttPrimary" id="changeStatus">
                            <i class="fa fa-exchange" aria-hidden="true"></i> Request For Close
                        </button>
                    <?php endif; ?>
...
<!-- Request for close-->
<div class="modal fade in" id="requestForCloseModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="largeModalLabel">Request for close</h5>
            </div>
            <form id="form_validation" >
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" align="center">
                            <h3>Are You Sure ?</h3>
                            <br>
                            <h4>You want to closing request to NIC Manager </h4>
                            <hr>
                        </div>
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <br>
                ...
                <div class="modal-footer">
                    <button type="button" id="requestForCloseModalYes" class="btn btn-primary waves-effect">YES, REQUEST</button>
                    <button type="button" id="requestForCloseModalNo" class="btn btn-danger waves-effect" data-dismiss="modal">CLOSE</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    {
        $('#requestForCloseModal').modal('show');
    });

    $(document).on('click','#requestForCloseModalNo',function ()
    {
        $('#requestForCloseModal').modal('hide');
    });
    $(document).on('click','#requestForCloseModalYes',function ()
    {
        ...
        $.ajax({
            url: BASE_URL + "/request-to-close-dev",
            type: "post",
            dataType: "json",
