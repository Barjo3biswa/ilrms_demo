...
<div class="container mb-5">
  <table class="table table-hover table-bordered" style="border-color:black" id="mouz_dist_wise_table">
    ...
    <tbody>    
        <?php foreach ($circle_wise_applications_info as $row): ?>
            <tr>
                <th scope="row">
                  <a href="<?=base_url()?>index.php/EkhajanaMouzadariDashboard/mouzaWiseInfo/<?=$row['dist_code']?>/<?=$row['subdiv_code']?>/<?=$row['cir_code']?>" target="_circle_mouz_dash">
                    <?=$row['cir_name']?>
                  </a>                  
                </th>
