    ...
    function alpineData() {
        return {
            forwardTo(id,dist_code,proposal_no) {
                ...
                $.confirm({
                    title: 'Secretary (Survey & Settlement) Note',
                    content:'<textarea id="note" placeholder="Secretary (Survey & Settlement) Note" class="form-control" required ></textarea>',
                    type: 'orange',
                    columnClass: 'custom-confirm-popup',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                            text: 'Forward to Joint Secretary',
                            btnClass: 'btn-success',
                            action: function() {
                                if (!$('#note').val()) {
                                    alert('Please enter the Secretary (Survey & Settlement) Note.');
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcSecretaryController/proposalForward',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
            ...
            revertedTo(id,dist_code,proposal_no) {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            text: 'Revert Back to Senior Most Secretary',
                            btnClass: 'btn-danger',
                            action: function() {
                                if (!$('#note').val()) {
                                    alert('Please enter the Secretary, (Survey & Settlement) Note');
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcSecretaryController/proposalRevert',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        (<?php if($type == 'pending'): ?>Pending at <?php elseif ($type == 'verified'): ?> Forwarded by <?php elseif ($type == 'reverted'): ?> Reverted at <?php endif; ?>
        Proposals at Secretary (Survey & Settlement) )</div>
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <?php foreach ($proposal as $p): ?>
                            <tr>
                                ...
                                <?php if($type == 'pending' || $type == 'reverted'): ?>
                                    <td>
                                        <button type="button" x-on:click="forwardTo('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="btn btn-sm btn-success text-white"><i class="fa fa-forward"></i> Forward to Joint Secretary, Survey & Settlement </button><br>
                                        <button type="button" x-on:click="revertedTo('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="btn btn-sm btn-danger text-white mt-1"><i class="fa fa-backward"></i> Revert Back to Senior Most Secretary </button>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
