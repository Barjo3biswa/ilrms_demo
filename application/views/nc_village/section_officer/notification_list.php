    ...
    function alpineData() {
        return {
            ...
            generateNotification(notification_id,dist_code,proposal_no,type) {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/viewAsstSectionOfficerNotification',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
            ...
            forwardTo() {
                var self = this;
                if (!this.note) {
                    alert('Please enter the Section Officer Survey & Settlement R&DM Department Note');
                    return;
                }
                this.is_forwarding = true;
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/notificationForward',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            viewNotification(notification_id,dist_code) {
                ...
                this.is_forwarding = true;
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/viewVerifiedNotification',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/notificationRevert',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        ...
        Section Officer (Survey & Settlement) R&DM Department )
    </div>
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <?php foreach ($notification as $key=>$p): ?>
                            <tr>
                                ...
                                <?php if($type == 'pending' || $type =='reverted'): ?>
                                    <td>
                                        <button type="button" x-on:click="generateNotification('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>','forward')" class="btn btn-sm btn-primary text-white"><i class="fa fa-forward"></i> Forward to Joint Secretary</button><br>
                                        <button type="button" x-on:click="generateNotification('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>','revert')" class="btn btn-sm btn-danger text-white mt-1"> <i class="fa fa-backward"></i> Revert to Assistant Section Officer</button>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
            ...
            <div class="modal-content">
                ...
                <div class="p-2 pt-0" style="justify-content:start;">
                    <div class="form-group">
                        <label for="" class="form-label" style="font-weight: bold;">Section Officer Survey & Settlement R&DM Department Note <span style="color:red;">*</span></label>
                        <textarea x-model="note" placeholder="Section Officer Survey & Settlement R&DM Department Note" rows="2"
                        ...
                        <div class="pt-2" id="modal_footer">
                            <button x-show="type == 'forward'" x-on:click="forwardTo" type="button" class="btn btn-primary"><i class="fa fa-check"></i>
                                Verify & Forward to Joint Secretary (Survey & Settlement)
                            </button>
                            <button x-show="type == 'revert'" x-on:click="revertTo" type="button" class="btn btn-danger"><i class="fa fa-backward"></i>
                                Revert to Assistant Section Officer (Survey & Settlement)
                            </button>
