    ...
    function alpineData() {
        return {
            forwardTo(id,dist_code,proposal_no) {
                ...
                $.confirm({
                    title: 'Section Officer (Survey & Settlement) Note',
                    content:'<textarea id="note" placeholder="Section Officer (Survey & Settlement) Note" class="form-control" required ></textarea>',
                    type: 'orange',
                    columnClass: 'custom-confirm-popup',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                            text: 'Forward to Assistant Section Officer',
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/proposalForward',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
            ...
            revertedTo(id,dist_code,proposal_no) {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            text: 'Revert Back to Joint Secretary',
                            btnClass: 'btn-danger',
                            action: function() {
                                if (!$('#note').val()) {
                                    alert('Please enter the Section Officer (Survey & Settlement) Note');
                                    self.is_forwarding = false;
                                    return;
                                }
                                var note = $('#note').val();
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcSectionOfficerController/proposalRevert',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        (<?php if($type == 'pending'): ?>Pending at
        <?php elseif ($type == 'verified'): ?> Forwarded by
        <?php elseif ($type == 'reverted'): ?> Reverted Proposal, <?php endif; ?>
        Section Officer (Survey & Settlement) R&DM Department )</div>
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <?php foreach ($proposal as $p): ?>
                            <tr>
                                ...
                                <?php if($type == 'pending' || $type == 'reverted'): ?>
                                    <td>
                                        <button type="button" x-on:click="forwardTo('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="btn btn-sm btn-success text-white"><i class="fa fa-forward"></i> Forward to Assistant Section Officer, Survey & Settlement </button>
                                        <button type="button" x-on:click="revertedTo('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="btn btn-sm btn-danger text-white mt-1"><i class="fa fa-backward"></i> Revert Back to Joint Secretary </button>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
