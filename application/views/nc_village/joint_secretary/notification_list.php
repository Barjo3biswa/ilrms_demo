    ...
    function alpineData() {
        return {
            ...
            generateNotification(notification_id,dist_code,proposal_no,type) {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcJointSecretaryController/viewSectionOfficerNotification',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcJointSecretaryController/notificationForward',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            viewNotification(notification_id,dist_code) {
                ...
                this.is_forwarding = true;
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcJointSecretaryController/viewVerifiedNotification',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcJointSecretaryController/notificationRevert',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        ...
        Joint Secretary (Survey & Settlement), R&DM Department )
    </div>
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <?php foreach ($notification as $key=>$p): ?>
                            <tr>
                                ...
                                <?php if($type == 'pending' || $type == 'reverted'): ?>
                                    <td>
                                        <button type="button" x-on:click="generateNotification('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>','forward')" class="btn btn-sm btn-primary text-white"> <i class="fa fa-forward"></i> Forward to Secretary</button><br>
                                        <button type="button" x-on:click="generateNotification('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>','revert')" class="btn btn-sm btn-danger text-white mt-1"> <i class="fa fa-backward"></i> Revert to Section Officer</button>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
            ...
            <div class="modal-content">
                ...
                <div class="p-2 pt-0" style="justify-content:start;">
                    <div class="form-group">
                        <label for="" class="form-label" style="font-weight: bold;">Joint Secretary (Survey & Settlement), R&DM Department Note <span style="color:red;">*</span></label>
                        <textarea x-model="note" placeholder="Joint Secretary (Survey & Settlement), R&DM Department Note" rows="2"
