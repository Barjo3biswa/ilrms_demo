...
<div class="container-fluid">
    ...
    <h4 class="text-center">  <?php if ($type == 'verified_lm') {echo ('Circle Wise Number of LM Verified NC Villages for ') . $dist_name;} else if ($type == 'certified_co') {echo ('Circle Wise Number of CO Certified NC Villages for ') . $dist_name;} else if ($type == 'digi_signature_dc') {echo ('Circle Wise Number of DC Certified NC Villages for ') . $dist_name;}?></h4>
                ...
                <?php if (!empty(($circles))): ?>
                    <?php foreach ($circles as $data): ?>
                        <?php if ($data["count"] != 0): ?>
                            <tr>
                                <td>
                                    <a href="<?=base_url('index.php/nc_village/NcDashboardController/getNcMouzasByStatus/') . $type . '/' . $data['dist_code'] . '/' . $data['subdiv_code'] . '/' . $data['cir_code']?>" class='' title=""><?php echo trim($data['loc_name']); ?></a>
                                </td>
