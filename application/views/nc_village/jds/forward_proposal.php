...
<script>
    function alpineData() {
        return {
            ...
            init() {
                var self = this;
                var pending_proposal = '<?= json_encode($pending_proposal) ?>';
                this.pending_proposal = JSON.parse(pending_proposal);
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE (Forwarded By ADLR)</div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div>
                            <h5> List of pending proposals</h5>
                        </div>
                    </div>
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <template x-for="(ap,index) in pending_proposal " :key="index">
                            <tr>
                                <td x-text="++index"></td>
                                ...
                                <td>
                                    <button class="btn btn-success" x-on:click="revertBack(ap.id,ap.proposal_no,ap.dist_code)"><i class="fa fa-backward"></i> Revert Back to ADLR</button>
                                </td>
                            </tr>
                        </template>
                        <tr x-show="pending_proposal.length == 0">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <template x-for="(rp,index) in revert_proposal " :key="index">
                            <tr>
                                <td x-text="++index"></td>
                                <td x-text="rp.proposal_no"></td>
                                <td x-text="rp.created_at"></td>
                                <td x-text="rp.adlr_note"></td>
                                <td x-text="rp.jds_note"></td>
                                <td>
                                <a class="btn btn-sm btn-info text-white" x-bind:href="'<?= base_url() ?>index.php/nc_village/NcCommonMyController/getVillagesProposalWise?proposal_id=' + rp.id+'&proposal_no='+rp.proposal_no+'&dist_code='+rp.dist_code">View <i class=" fa fa-eye"></i></a>
                                </td>
                            </tr>
                        </template>
