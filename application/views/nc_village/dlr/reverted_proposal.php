    ...
    function alpineData() {
        return {
            ...
            signProposal() {
                ...
                var self = this;
                self.loadPreviewForSign('<?= base_url(); ?>index.php/nc_village/NcDlrController/getProposalBaseReverted/'+self.proposal_no);
            },
            signProposalConfirm() {
            ...
            storeSignedProposal(jsonData, sign_key) {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcDlrController/storeSignedProposalReverted',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
            ...
            revertedTo(notification_id,proposal_id,dist_code) {
                var self = this;
                ...
                $.confirm({
                    title: 'DLRS Note',
                    content:'<textarea id="note" placeholder="DLRS Note" class="form-control" required ></textarea>',
                    type: 'orange',
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                if (!$('#note').val()) {
                                    alert('Please enter the DLRS Note.');
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcDlrController/revertBackProposal',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcDlrController/revertBackDcProposal',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function () {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcDlrController/getProposalBase64',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            justForwardTo(proposal_id,dist_code,proposal_no) {
                ...
                $.confirm({
                    title: 'Director of Land Records and Surveys Note',
                    content:'<textarea id="note" placeholder="Director of Land Records and Surveys Note" class="form-control" required ></textarea>',
                    columnClass: 'custom-confirm-popup',
                    type: 'orange',
                    typeAnimated: true,
                    buttons: {
                            ...
                            action: function() {
                                ...
                                buttons: {
                                    Confirm: {
                                        ...
                                        action: function () {
                                            $.ajax({
                                                url: '<?= base_url(); ?>index.php/nc_village/NcDlrController/proposalForward',
                                                method: "POST",
                                                async: true,
                                                dataType: 'json',
                                                data: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        (List of Reverted Proposal From ADLR, Senior Most Secretary)
    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div>
                            <h5> List of Reverted Proposal From ADLR, Senior Most Secretary</h5>
                        </div>
                    </div>
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <tbody>
                        <?php $key = 0; foreach ($proposal as $key=>$pp): ?>
                            <tr>
                                <td><?= ++$key ?></td>
                                <td><?= $pp->proposal_no ?></td>
                                <td>
                                    <a class="btn btn-sm btn-info text-white"
                                       href="<?=base_url('index.php/nc_village/NcCommonController/viewProposal?id=').$pp->proposal_no;?>"
                                ...
                                <td><?= $pp->adlr_note ?></td>
                                <td>
                                    <button type="button" x-on:click="forwardTo('<?= $pp->proposal_no?>','<?= $pp->dist_code?>')"
                                        class="btn btn-sm btn-success text-white"><i class="fa fa-pencil"></i>
                                        Sign & Forward to Senior Most Secretary
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        ...
                        <?php foreach ($notification as $key=>$p): ?>
                            <tr>
                                ...
                                <td><?= $p->ps_note ?></td>
                                <td>
                                    <button type="button" x-on:click="revertedTo(<?= $p->id.','.$p->proposal_id.','.$p->dist_code ?>,)"
                                            class="btn btn-sm btn-danger text-white"><i class="fa fa-backward"></i> Revert To DC
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                        <?php if(count($notification) + count ($proposal) + count($ps_proposal) == 0): ?>
                            <tr>
            ...
            <div class="modal-dialog modal-xl" style="width: 100% !important;">
                <div class="modal-content">
                    ...
                    <div class="p-3" x-show="sign_type !='proposal'">
                        <label for="" class="form-label" style="font-weight: bold;">Director of Land Records and Survey Note <span style="color:red;">*</span></label>
                        <textarea x-model="note" placeholder="Director of Land Records and Survey Note" rows="2" class="form-control"></textarea>
                    </div>
