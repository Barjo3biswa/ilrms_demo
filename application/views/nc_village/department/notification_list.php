...
<script>
    function alpineData() {
        return {
            ...
            dist_code:'',
            notification: '',
            notification_revert_back: '',
            notification_id: '',
            note: '',
            notification_data:'',
            is_forwarding:false,
            'is_loading': false,
            generateNotification(notification_id,dist_code,proposal_no) {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcDepartController/viewSecNotification',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
            ...
            revertBack(notification_id,dist_code,proposal_no) {
                this.notification_id = notification_id;
                this.dist_code = dist_code;
                this.proposal_no = proposal_no;
                var self = this;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status == '0') {
                            ...
                            var elementt = document.getElementById("editor_revert_back");
                            if (!!elementt && !!elementt.editorrevertback) {
                                // Editor already exists, replace its content
                                elementt.editorrevertback.setData(data.data);
                            }else {
                                CKEDITOR.ClassicEditor.create(elementt, {
                                    toolbar: {
                                        items: [
                                            'heading', '|',
                                            'bold', 'italic', 'strikethrough', 'underline', 'code', 'subscript', 'superscript', 'removeFormat', '|',
                                ...
                                }).then(editorrevertback => {
                                    editorrevertback.setData(data.data);
                                    editorrevertback.model.document.on('change:data', () => {
                                        self.notification_revert_back = editorrevertback.getData();
                                    })
                                    elementt.editorrevertback = editorrevertback;
                                })
                            }
                            $('#modal_revert_back').modal('show');
                        }
                        self.is_forwarding = false;
                    },
                    error: function(error) {
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcDepartController/notificationForward',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
            ...
            revertBacktoDLRS() {
                ...
                $.confirm({
                    title: 'Confirm',
                    content: 'Please confirm to Revert Back',
                    type: 'orange',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                            ...
                            action: function() {
                                $.ajax({
                                    url: '<?= base_url(); ?>index.php/nc_village/NcDepartController/notificationRevertBackToDLRS',
                                    method: "POST",
                                    async: true,
                                    dataType: 'json',
                                    data: {
                                    ...
                                    success: function(data) {
                                        if (data != 'Y') {
                                            ...
                                            $.confirm({
                                                title: 'Success',
                                                content: 'Reverted to DLRS successfully.',
                                                type: 'green',
                                                typeAnimated: true,
                                                buttons: {
                                                    Ok: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE
        ( <?php if($type == 'pending'): ?>Draft Notification Pending at <?php elseif ($type == 'verified'): ?>
            Draft Notification Forwarded by <?php elseif ($type == 'reverted'): ?>
            Reverted Draft Notification <?php endif; ?>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        <div>
                            <h5> List of <?php if($type == 'pending'): ?>Pending <?php elseif ($type == 'verified'): ?> Forwarded
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <?php foreach ($notification as $key=>$p): ?>
                            <tr>
                                ...
                                <?php if($type == 'pending' || $type =='reverted'): ?>
                                    <td>
                                        <button type="button" x-on:click="generateNotification('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="btn btn-sm btn-primary text-white"><i class="fa fa-forward"></i>
                                            Forward to Honourable Minister, R&DM Department </button>
                                 <br>
                                        <button type="button" x-on:click="revertBack('<?= $p->id ?>','<?= $p->dist_code ?>','<?= $p->proposal_no ?>')" class="mt-2 btn btn-sm btn-danger text-white"><i class="fa fa-backward"></i> Revert to DLRS </button>
                                    </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
            ...
            <div class="modal-content">
                ...
                <div class="p-2 pt-0" style="justify-content:start;">
                    <div class="form-group">
                        ...
                        <textarea x-model="note" placeholder="Senior Most Secretary, R&DM Department Note" rows="2"
                                  class="form-control"></textarea>
                        <div class="pt-2" id="modal_footer">
                            <button x-on:click="forwardTo" type="button" class="btn btn-primary"><i class="fa fa-check"></i>
                                Verify & Forward to Honourable Minister, R&DM Department
                            </button>
                            <button type="button" class="btn btn-danger" id="closeBtn" data-dismiss="modal">
                                <i class='fa fa-close'></i> Close
                            </button>
...
<!--    Revert Back-->
    <div class="modal" id="modal_revert_back" tabindex="-1" aria-labelledby="modal_revert_back" data-backdrop="static" data-keyboard="false" aria-hidden="true">
        <div class="modal-dialog modal-xl" style="width: 100% !important;">
            <div class="modal-content">
                <div class="modal-header p-2">
                    <h5 class="modal-title"> <i class="fa fa-file"></i> Draft Notification Generated of NC Village as a Cadastral Village <span class="text-danger font-weight-bold">(Revert to DLRS)</span></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <textarea name="content" id="editor_revert_back"></textarea>
                </div>
                <hr>
                <div class="p-2 pt-0" style="justify-content:start;">
                    <div class="form-group">
                        ...
                        <textarea x-model="note" placeholder="Senior Most Secretary, R&DM Department Note" rows="2"
                                  class="form-control"></textarea>
                        <div class="pt-2" id="modal_footer">
                            <button x-on:click="revertBacktoDLRS" type="button" class="btn btn-danger"><i class="fa fa-backward"></i>
                                Revert to DLRS
                            </button>
