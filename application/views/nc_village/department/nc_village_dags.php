    ...
    function alpineData() {
        return {
            ...
            getDags() {
                ...
                $.ajax({
                    url: '<?= base_url(); ?>index.php/nc_village/NcDepartController/getDags',
                    method: "POST",
                    async: true,
                    dataType: 'json',
                    data: {
            ...
            certifyVillage() {
                if (!this.depart_note) {
                    alert('Please enter the Department Note');
                    return;
                }
                var self = this;
                this.is_loading = true;
                $.confirm({
                    title: 'Confirm',
                    content: 'Please confirm to submit this village.',
                    type: 'orange',
                    typeAnimated: true,
                    buttons: {
                        Confirm: {
                                ...
                                $.confirm({
                                    ...
                                    buttons: {
                                        Confirm: {
                                            ...
                                            action: function() {
                                                $.ajax({
                                                    url: '<?= base_url(); ?>index.php/nc_village/NcDepartController/certifyVillage',
                                                    method: "POST",
                                                    async: true,
                                                    dataType: 'json',
                                                    data: {
                                                    ...
                                                    success: function(data) {
                                                        ...
                                                        if (data.submitted == 'Y') {
                                                            $.confirm({
                                                                title: 'Success',
                                                                content: 'The Chitha and Map of this village have been verified.',
                                                                type: 'green',
                                                                typeAnimated: true,
                                                                buttons: {
                                                                    Ok: {
                ...
                <div class="card-body">
                    <h5> DAGS <span id="application_no" class="bg-gradient-danger text-white">
                        ...
                        <?php if($approve_proposal): ?>
                        	<span>
	                             <a href="<?=base_url()?>index.php/nc_village/NcCommonController/viewProposal?id=<?=$approve_proposal->proposal_no?>"
	                                class="btn btn-secondary py-2" style="color: white" target="_blank"
                    ...
                    <div class="border-top border-dark py-3" x-show="nc_village.depart_verified == 'Y'">
                        <div class="form-group pb-2">
                            <label for="" class="form-label" style="font-weight: bold;">Department Note <span style="color: red">*</span></label>
                            <textarea rows="2" x-text="nc_village.depart_note"
                    ...
                    <div class="border-top border-dark py-3" x-show="nc_village.depart_verified != 'Y'">
                        <div class="form-group pb-2">
                            <label for="" class="form-label" style="font-weight: bold;">Department Note <span style="color: red">*</span></label>
                            <textarea x-model="depart_note" placeholder="Department Note" rows="2" class="form-control"></textarea>
                        </div>
                        <button x-on:click="certifyVillage" class="btn btn-success" type="button"><i class="fa fa-check"></i> Submit </button>
                    </div>

                    <div x-show="nc_village.depart_verified == 'Y'">
                        <h5 class="text-success">The Department has already submitted.</h5>
                    </div>
                    <?php if(sizeof($map) == 0): ?>
