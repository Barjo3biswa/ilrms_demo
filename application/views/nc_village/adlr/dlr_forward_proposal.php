...
<script>
    function alpineData() {
        return {
            ...
            init() {
                var self = this;
                var approve_proposal = '<?= json_encode($proposal) ?>';
                this.approve_proposal = JSON.parse(approve_proposal);
            },
            forwardTo(proposal_id,proposal_no,dist_code) {
                var self = this;
                ...
                $.confirm({
                    ...
                    buttons: {
                        Confirm: {
                            text: 'Forward to Joint Director of Surveys',
                                                ...
                                                $.ajax({
                                                    ...
                                                    success: function(data) {
                                                        if (data.status == '0') {
                                                            ...
                                                            $.confirm({
                                                                title: 'Success',
                                                                content: 'Proposal successfully reverted back to DLRS.',
                                                                type: 'green',
                                                                typeAnimated: true,
                                                                buttons: {
                                                                    Ok: {
...
<div class="col-lg-12 col-md-12 p-2" x-data="alpineData()">
    <div class="text-center p-2 mb-2" style="font-size:18px; font-weight: bold; background-color: #4298c9; color: white">NC VILLAGE (Forwarded By DLRS)</div>
                    ...
                    <table class="table table-hover table-sm table-bordered table-stripe">
                        ...
                        <template x-for="(ap,index) in approve_proposal " :key="index">
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-primary" x-on:click="forwardTo(ap.id,ap.proposal_no,ap.dist_code)"><i class="fa fa-forward"></i> Forward to JDS</button>
                                </td>
                                <td>
