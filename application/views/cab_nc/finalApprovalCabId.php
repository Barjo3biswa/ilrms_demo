...
<script type="text/javascript">
  ...
  $(document).ready( function () {
    ...
    function load_data(selectDistrict)
    {
      var table = $('#dataTableCab').DataTable({
        ...
        'ajax':{
          url: base_url+'index.php/CabController/getCabIdByUserDistrictFinalApprovalNC',
          type:'POST',
  ...
  $('#saveAndProcess').click(function(){
    ...
    if (selectedList.length > 0)
    {
      ...
      $.ajax({
        url: BASE_URL + "/CabController/finalCabApprovalSaveNC",
        type: "post",
        dataType: "json",
        contentType: "application/json",
        success: function (data) {
  ...
  $('#generateNotificationPost').on('submit', function(event){
    ...
    var formData = $(this).serialize();
        ...
        $.ajax({
            type        : 'POST', 
            url         : base_url +'CabController/GenerateNotificationNC', 
            data        : formData, 
            // dataType    : 'json', 
            encode      : true,
            success: function(data){
