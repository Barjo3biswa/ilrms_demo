	...
	<div class="col-lg-10">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb p-3 text-white">
				<li class="breadcrumb-item font-weight-bold"><a href="<?= base_url().'zonal-dag-district-wise'?>">Zonal Info</a></li>
		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb p-3 text-white">
				<li class="breadcrumb-item font-weight-bold">
					<a href="<?= base_url().'zonal-dag-district-wise'?>">
...
<div class="p-2 bg-primary text-center text-white h5 shadow-sm border border-white rounded"><b>Zonal Dag Info Circle Wise</b>
</div>
	...
	<div class="tab-content">
		<div class="card-body">
			<table class="table table-hover text-center tab1">
				...
				<?php
				$i =1;
				foreach($circleZonalDagCount as $key=>$row) :
					?>
					<tr>
						<td><?= $i++ ?></td>
						<td><?= $this->utilityclass->cir_name($row->district_code,$row->subdiv_code,$row->cir_code) ?></td>
						<td><?= $row->zonal_dags ?></td>
						<td><?= $row->chitha_dags ?></td>
						<td><?= $row->pending_dags ?></td>
						<td align="center">
							<a href="<?= base_url().'zonal-dag-village-wise?dist_code='.$row->district_code.'&subdiv='.$row->subdiv_code.'&cir_code='.$row->cir_code?>">
