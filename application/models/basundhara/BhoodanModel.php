...
class BhoodanModel extends CI_Model
{
    ...
    public function alldepartmentRequest($service)
    {
        ...
        $sql = " Select * ,CASE
                ...
                WHEN (service_code = '25') THEN 'SETTLEMENT NC KHAS LAND'
                END AS service
    ...
    {
        $sql = "select * from cab_id_list where dist_code =? and user_code =? and status in (?, ?) and nc=?";
        $data  = $this->db->query($sql, array($dist_code,$user_code,GENERATED_CAB_ID,ADD_CASES_UNDER_CAB_ID,'Y'));
        return $data;
    }
    ...
    public function getAllCasesUnderDepartmentAll($dbb,$start, $length, $order, $service_code,$meeting_no,$verification,$pullRequest)
    {
        ...
        $dbb->where('sb.service_code', BHOODAN_LAND_ID);
    ...
    public function getMeetingListByDist($dbb,$dist_code)
    {
        ...
        $data  = $dbb->query($sql, array($dist_code, DIGITAL_SIGN_STATUS, MB_DEPUTY_COMM, ADD_CASES_TO_CAB_MEMO,BHOODAN_LAND_ID));
        // echo $dbb->last_query();
        // die;
        return $data;
    }
    ...
    public function getDeptUserDistListWithCaseCount()
    {
        ...
        foreach ($dist_list as $key => $value) {
            ...
            $sqlForCaseCountAgainstDistrict = $this->db2->query("SELECT count(sc.*) as total FROM 
                ...
                ",array($value->dist_code,MB_DEPUTY_COMM,MB_DEPARTMENT,ADD_CASES_TO_CAB_MEMO,BHOODAN_LAND_ID,DIGITAL_SIGN_STATUS))->row();
            ...
            // log_message('error','SKHAS======='.$this->db2->last_query());
            $dist_list[$key]->case_count = $caseCount;
        }
        return $dist_list;
    }
    ...
    public function getAllRevertedCasesUnderDepartmentAll($dbb,$start, $length, $order, $service_code)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $dbb->from('settlement_basic');
            $dbb->where('dept_revert', 1);
            $dbb->where('service_code', BHOODAN_LAND_ID);
            if ($service_code != NULL) {
                $dbb->where('service_code', $service_code);
            }
            $data['total_records'] = $dbb->count_all_results();
    ...
    public function getDeptUserDistListWithRevertedCaseCount()
    {
        ...
        foreach ($dist_list as $key => $value) {
            ...
            $sqlForCaseCountAgainstDistrict = $this->db2->query("SELECT count(*) as total FROM settlement_basic WHERE dist_code = ? and dept_revert =? and service_code= ?
                ",array($value->dist_code,1,BHOODAN_LAND_ID))->row();
            if(!empty($sqlForCaseCountAgainstDistrict) && isset($sqlForCaseCountAgainstDistrict) && $sqlForCaseCountAgainstDistrict != null){
                $caseCount = $sqlForCaseCountAgainstDistrict->total;
            }
            $dist_list[$key]->case_count = $caseCount;
    ...
    public function applicationStatusUpdateBulk($application_no,$case,$rmk,$status,$task,$pen){
        $apilink=API_LINK;
        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, $apilink."applicationStatusUpdateBulkMb3ServiceDpt");
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST,  2);
        curl_setopt($curl_handle, CURLOPT_CUSTOMREQUEST, "POST");
    ...
    public function addIntoRevertList($case_no,$revert_remarks,$service_code,$user_code,$dist_code)
    {
        $insertData=array(
            'case_no'=>$case_no,
            'user_code'=>$user_code,
        ...
        $insertDataStatus = $this->db->insert('cab_memo_revert_list', $insertData);
        if($insertDataStatus != 1)
        {
            return false;
        }
        else
