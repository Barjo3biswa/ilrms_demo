<?php
class BasundharaMb2Perpetualmodel extends CI_Model
{
    ...
    {
        $cab_type ='PERPETUAL';
        $sql = "select * from cab_id_list where dist_code =? and user_code =? and cab_type=? and status in (?, ?)";
        $data  = $this->db->query($sql, array($dist_code,$user_code,$cab_type,GENERATED_CAB_ID,ADD_CASES_UNDER_CAB_ID));
        return $data;
    }
    ...
    public function getAllCasesUnderDepartmentAll($dbb,$start, $length, $order, $service_code,$meeting_no,$verification,$pullRequest)
    {
        ...
        $dbb->where_in('sb.service_code',array('13','16','18')); // occupancy tenant, cultivation, khas land
        $dbb->where('sb.review_flag',0);
        ...
        if ($query->num_rows() > 0) {
            ...
            $dbb->where('sb.cab_memo_prepared', ADD_CASES_TO_CAB_MEMO);
            // $dbb->where('sb.dept_approval IS NULL');
            $dbb->where_in('sb.service_code',array('13','16','18'));
            $dbb->where('sb.review_flag',0);
            $dbb->where('sm.digital_sign_status', DIGITAL_SIGN_STATUS);
    ...
    public function getMeetingListByDist($dbb,$dist_code)
    {
        ...
        $sql = "select distinct ml.id,ml.meeting_name from proposal_meeting_list ml 
                    ...
                    and review_flag=0  and sb.service_code in ('13','16','18') and ml.digital_sign_status =? and sb.from_office = ? and sb.cab_memo_prepared =? ";
        $data  = $dbb->query($sql, array($dist_code, DIGITAL_SIGN_STATUS, MB_DEPUTY_COMM, ADD_CASES_TO_CAB_MEMO));
        //return $dbb->last_query();
        return $data;
    }
    ...
    public function getDeptUserDistListWithCaseCount()
    {
        ...
        foreach ($dist_list as $key => $value) {
            ...
            if(IS_PRODUCTION == 0 && $value->dist_code !='15') {
                $caseCount = 0;
            ...
            $sqlForCaseCountAgainstDistrict = $this->db2->query("SELECT count(sc.*) as total FROM 
                ...
                where  sb.dist_code = ? and sb.review_flag ='0' and sb.from_office =? and sb.pending_officer =? and sb.cab_memo_prepared = ? and sb.service_code in ('13','16','18')  and sm.digital_sign_status = ?
