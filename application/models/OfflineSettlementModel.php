<?php
class OfflineSettlementModel extends CI_Model
{
    ...
    function getSettlementProceeding($case_no)
    {
        ...
        $query = "SELECT * FROM settlement_proceeding WHERE case_no = '$case_no' ORDER BY proceeding_id DESC";
        $data = $this->db2->query($query)->result();
        return $data;
    }
    ...
    {
        $sql = "select * from cab_id_list where dist_code =? and user_code =? and status in (?, ?)";
    ...
    public function getCasesUnderCabMemo($dbb,$cab_memo_id)
    {
        ...
        $sql = "select case_no from cab_memo_list where cab_id = ?";
    ...
    public function getAllCasesUnderDepartmentAll($dbb,$start, $length, $order, $service_code,$meeting_no,$verification,$pullRequest)
    {
        ...
        $dbb->select('sc.*,sm.meeting_name,sm.id as meeting_id, sb.dept_approval,sb.verified_by_asst,sb.service_code,sb.pull_request,sb.verified_ast_remarks,sb.dept_revert');
        $dbb->from('offline_settlement_case_details as sc');
        $dbb->join('settlement_basic sb', 'sb.case_no = sc.case_no');
        $dbb->join('offline_meeting_cases sl', 'sl.case_no = sc.case_no');
        $dbb->join('offline_meeting_list sm', 'sm.id = sl.meeting_id');
        ...
        $dbb->where('sb.service_code', OFFLINE_SETTLEMENT_ID);
    ...
    public function getMeetingListByDist($dbb,$dist_code)
    {
        ...
        $sql = "select distinct ml.id,ml.meeting_name from offline_meeting_list ml 
                    join offline_meeting_cases pl on pl.meeting_id = ml.id 
                    join  settlement_basic sb on pl.case_no = sb.case_no where ml.dist_code = ? 
                    and sb.from_office = ? and sb.cab_memo_prepared =? ";
        $data  = $dbb->query($sql, array($dist_code, MB_DEPUTY_COMM, ADD_CASES_TO_CAB_MEMO));
        return $data;
    }
    ...
    public function getAllCasesUnderAssistant($dbb,$start, $length, $order, $service_code,$meeting_no)
    {
        ...
        $dbb->select('sc.*,sm.meeting_name,sm.id, sb.dept_approval,sb.verified_by_asst,sb.service_code');
        $dbb->from('offline_settlement_case_details as sc');
        $dbb->join('settlement_basic sb', 'sb.case_no = sc.case_no');
        $dbb->join('offline_meeting_cases sl', 'sl.case_no = sc.case_no');
        $dbb->join('offline_meeting_list sm', 'sm.id = sl.meeting_id');
    ...
    public function getCasesCountByDistMeeting($dbb,$meeting_id)
    {
        ...
        $sql = "select sc.case_no, sb.pull_request
                        from offline_settlement_case_details as sc 
                        join settlement_basic sb on sb.case_no = sc.case_no
                        join offline_meeting_cases sl on sl.case_no = sc.case_no
                        join offline_meeting_list sm on sm.id = sl.meeting_id
                        where sb.pending_officer = ?  and (sb.dept_approval is NULL or sb.dept_approval='R' or sb.dept_approval='') 
                        and sm.id = ?";
        $data  = $dbb->query($sql, array($pendingofficer,$meeting_id));
        return $data;
    }
    ...
    public function getAllCasesFromMemoForFinalApproval($dbb,$cab_id,$user_code)
    {
        ...
        $sql = "select * from cab_memo_list where cab_id =? and user_code =?   order by id";
        $data  = $dbb->query($sql, array($cab_id,$user_code));
        return $data;
    }

    public function digitalSignedStatusOfCabId($dbb,$cab_id)
    {
        $sql = "select notification_digital_sign_status from cab_id_list where cab_id =? and offline = 'Y'";
    ...
    public function getDeptUserDistListWithCaseCount()
    {
        ...
        foreach ($dist_list as $key => $value) {
            ...
            $sqlForCaseCountAgainstDistrict = $this->db2->query("SELECT count(sc.*) as total FROM 
                offline_settlement_case_details sc 
                inner join settlement_basic sb on sb.case_no = sc.case_no 
          
                inner join offline_meeting_cases sm on sm.case_no = sc.case_no
                inner join offline_meeting_list om on om.id = sm.meeting_id
                where  sb.dist_code = ? and (sb.from_office =? or sb.from_office =?) and sb.pending_officer =? and sb.cab_memo_prepared = ? and sb.service_code = ? and om.meeting_status= ?",array($value->dist_code,MB_DEPUTY_COMM,MB_ADD_DEPUTY_COMM,MB_DEPARTMENT,ADD_CASES_TO_CAB_MEMO,OFFLINE_SETTLEMENT_ID,2))->row();
            ...
            log_message('error','OSKHAS======='.$this->db2->last_query());
            $dist_list[$key]->case_count = $caseCount;
        }
        return $dist_list;
    }
    ...
    public function getDeptUserDistListWithRevertedCaseCount()
    {
        ...
        foreach ($dist_list as $key => $value) {
            ...
            $this->db2 = $this->dbswitch2($value->dist_code);
            $sqlForCaseCountAgainstDistrict = $this->db2->query("SELECT count(*) as total FROM settlement_basic WHERE dist_code = ? and dept_revert =? and service_code = ?
                ",array($value->dist_code,1,OFFLINE_SETTLEMENT_ID))->row();
            if(!empty($sqlForCaseCountAgainstDistrict) && isset($sqlForCaseCountAgainstDistrict) && $sqlForCaseCountAgainstDistrict != null){
                $caseCount = $sqlForCaseCountAgainstDistrict->total;
            }
            $dist_list[$key]->case_count = $caseCount;
    ...
    public function getMeetingProposalByCaseNo($dbb,$case)
    {
        $sql =    "SELECT spl.id as proposal_id,sml.id as meeting_id
                        FROM offline_settlement_case_details as spc
                        JOIN offline_meeting_cases AS spl ON spc.case_no = spl.case_no
                        JOIN offline_meeting_list AS sml ON sml.id = spl.meeting_id
                        WHERE spc.case_no='$case'";
